...
import { BluFeatureSetting } from "@shared/BluFeatureSetting"
import { DEFAULT_DICTATION_SETTINGS, DictationSettings } from "@shared/DictationSettings"
import { DEFAULT_PLATFORM, type ExtensionState } from "@shared/ExtensionMessage"
import { DEFAULT_FOCUS_CHAIN_SETTINGS } from "@shared/FocusChainSettings"
import { DEFAULT_MCP_DISPLAY_MODE } from "@shared/McpDisplayMode"
import { DEFAULT_PROMPT_ENGINEERING_SETTINGS } from "@shared/PromptEngineeringSettings"
import { EmptyRequest } from "@shared/proto/blu/common"
import type { OpenRouterCompatibleModelInfo } from "@shared/proto/blu/models"
import { type TerminalProfile } from "@shared/proto/blu/state"
import { convertProtoToBluMessage } from "@shared/proto-conversions/blu-message"
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	// View state
	showMcp: boolean
	hooksEnabled?: BluFeatureSetting
	mcpTab?: McpViewTab
	...
	setGlobalBluRulesToggles: (toggles: Record<string, boolean>) => void
	setLocalBluRulesToggles: (toggles: Record<string, boolean>) => void
...
export const ExtensionStateContextProvider: React.FC<{
	...
	const [state, setState] = useState<ExtensionState>({
		...
		promptEngineeringSettings: DEFAULT_PROMPT_ENGINEERING_SETTINGS,
		...
		globalBluRulesToggles: {},
		localBluRulesToggles: {},
				...
				if (response.stateJson) {
					try {
						...
						setState((prevState) => {
							...
							// HACK: Preserve bluMessages if currentTaskItem is the same
							if (stateData.currentTaskItem?.id === prevState.currentTaskItem?.id) {
								stateData.bluMessages = stateData.bluMessages?.length
									? stateData.bluMessages
									: prevState.bluMessages
							}
							...
							const newState = {
								...
								// Preserve promptEngineeringSettings if backend sends undefined
								promptEngineeringSettings:
									stateData.promptEngineeringSettings ?? prevState.promptEngineeringSettings,
							}
		...
		partialMessageUnsubscribeRef.current = UiServiceClient.subscribeToPartialMessage(EmptyRequest.create({}), {
			onResponse: (protoMessage) => {
				try {
					...
					const partialMessage = convertProtoToBluMessage(protoMessage)
					setState((prevState) => {
						// worth noting it will never be possible for a more up-to-date message to be sent here or in normal messages post since the presentAssistantContent function uses lock
						const lastIndex = findLastIndex(prevState.bluMessages, (msg) => msg.ts === partialMessage.ts)
						if (lastIndex !== -1) {
							const newBluMessages = [...prevState.bluMessages]
							newBluMessages[lastIndex] = partialMessage
							return { ...prevState, bluMessages: newBluMessages }
						}
						return prevState
					})
				} catch (error) {
	...
	const contextValue: ExtensionStateContextType = {
		...
		globalBluRulesToggles: state.globalBluRulesToggles || {},
		localBluRulesToggles: state.localBluRulesToggles || {},
		...
		setShowMcp,
		closeMcpView,
		setGlobalBluRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				globalBluRulesToggles: toggles,
			})),
		setLocalBluRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				localBluRulesToggles: toggles,
			})),
