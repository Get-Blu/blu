...
// Featured models for Blu provider
const featuredModels = [
	{
		id: "anthropic/claude-sonnet-4.5",
		description: "Recommended for agentic coding in Blu",
		label: "Best",
	},
	{
		id: "z-ai/glm-4.6:exacto",
		description: "Fast open-source model with improved performance in Blu",
		label: "Trending",
	},
	{
...
const FREE_BLU_MODELS = featuredModels.filter((m) => m.isFree)
...
const OpenRouterModelPicker: React.FC<OpenRouterModelPickerProps> = ({ isPopup, currentMode }) => {
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		const isBlu = selected.selectedProvider === "blu"
		// Makes sure "Free" featured models have $0 pricing for Blu provider
		if (isBlu && FREE_BLU_MODELS.some((fm) => fm.id === selected.selectedModelId)) {
			return {
				...selected,
				selectedModelInfo: {
	...
	const modelIds = useMemo(() => {
		...
		if (modeFields.apiProvider === "blu") {
			// For Blu provider: exclude :free models, but keep Minimax models
		...
		// For OpenRouter provider: exclude Blu-specific models
		return unfilteredModelIds.filter((id) => !id.startsWith("blu/"))
		...
		<div style={{ width: "100%" }}>
			...
			{hasInfo ? (
				...
				<p
					...
					If you're unsure which model to choose, Blu works best with{" "}
					<VSCodeLink
