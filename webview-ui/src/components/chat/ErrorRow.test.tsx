...
vi.mock("@/context/BluAuthContext", () => ({
	useBluAuth: () => ({
		bluUser: null,
	}),
	handleSignIn: vi.fn(),
...
// Mock BluError
vi.mock("../../../../src/services/error/BluError", () => ({
	BluError: {
		parse: vi.fn(),
	},
	BluErrorType: {
...
describe("ErrorRow", () => {
	...
	it("renders bluignore error", () => {
		const bluignoreMessage = { ...mockMessage, text: "/path/to/file.txt" }
		render(<ErrorRow errorType="bluignore_error" message={bluignoreMessage} />)

		expect(screen.getByText(/Blu tried to access/)).toBeInTheDocument()
	...
	describe("API error handling", () => {
			...
			const mockBluError = {
				...
				_error: {
					details: {
						...
						buy_credits_url: "https://app.getblu.in/dashboard",
					},
				},
			}

			const { BluError } = await import("../../../../src/services/error/BluError")
			vi.mocked(BluError.parse).mockReturnValue(mockBluError as any)
		...
		it("renders auth error with sign in button when user is not signed in", async () => {
			...
			expect(screen.getByText("Sign in to Blu")).toBeInTheDocument()
		})
		...
		it("renders PowerShell troubleshooting link when error mentions PowerShell", async () => {
			...
			expect(screen.getByRole("link", { name: "troubleshooting guide" })).toHaveAttribute(
				"href",
				"https://github.com/GarvAgnihotri/blu/wiki/TroubleShooting-%E2%80%90-%22PowerShell-is-not-recognized-as-an-internal-or-external-command%22",
			)
		})
		...
		it("falls back to regular error message when BluError.parse returns null", async () => {
			const { BluError } = await import("../../../../src/services/error/BluError")
			vi.mocked(BluError.parse).mockReturnValue(undefined)

			render(<ErrorRow apiRequestFailedMessage="Some API error" errorType="error" message={mockMessage} />)

			// When BluError.parse returns null, we display the raw error message for non-Blu providers
			// Since bluError is undefined, isBluProvider is false, so we show the raw apiRequestFailedMessage
