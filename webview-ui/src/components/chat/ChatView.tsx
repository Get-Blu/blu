...
import type { BluApiReqInfo, BluMessage } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { BooleanRequest, StringRequest } from "@shared/proto/blu/common"
...
const ChatView = ({ isHidden, showAnnouncement, hideAnnouncement, showHistoryView }: ChatViewProps) => {
	...
	const isProdHostedApp = userInfo?.apiBaseUrl === "https://app.getblu.in"
	const shouldShowQuickWins = isProdHostedApp && (!taskHistory || taskHistory.length < QUICK_WINS_HISTORY_THRESHOLD)

	//const task = messages.length > 0 ? (messages[0].say === "task" ? messages[0] : undefined) : undefined) : undefined
	const task = useMemo(() => messages.at(0), [messages]) // leaving this less safe version here since if the first message is not a task, then the extension is in a bad state and needs to be debugged (see Blu.abort)
	...
	const lastApiReqTotalTokens = useMemo(() => {
		const getTotalTokensFromApiReqMessage = (msg: BluMessage) => {
			...
			const { tokensIn, tokensOut, cacheWrites, cacheReads }: BluApiReqInfo = JSON.parse(msg.text)
	...
	const placeholderText = useMemo(() => {
		const text = task ? "Type a message..." : "Starts here..."
		return text
	}, [task])
