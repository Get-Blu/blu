...
import { BrowserAction, BrowserActionResult, BluMessage, BluSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: BluMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: BluMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: BluMessage[] = []
		let nextActionMessages: BluMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Blu is using the browser:" : "Blu wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
				...
				<div style={urlBarContainerStyle}>
					<div
						className={cn(
							"flex bg-input-background border border-input-border rounded-md px-1 py-0.5 min-w-0 text-description w-full justify-center",
							{
