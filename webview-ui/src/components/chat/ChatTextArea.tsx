...
import { EmptyRequest, StringRequest } from "@shared/proto/blu/common"
import { FileSearchRequest, FileSearchType, RelativePathsRequest } from "@shared/proto/blu/file"
import { UpdateApiConfigurationRequest } from "@shared/proto/blu/models"
import { PlanActMode, TogglePlanActModeRequest } from "@shared/proto/blu/state"
import { convertApiConfigurationToProto } from "@shared/proto-conversions/models/api-configuration-conversion"

import { VSCodeButton } from "@vscode/webview-ui-toolkit/react"
import { AtSignIcon, ChevronDownIcon, ChevronUpIcon, Images } from "lucide-react"
...
import { useBluAuth } from "@/context/BluAuthContext"
...
const ModeDropdownButton = styled.button<{ isActive?: boolean }>`
	display: flex;
	align-items: center;
	gap: 4px;
	padding: 4px 8px;
...
const ModeDropdownItem = styled.button<{ isSelected?: boolean }>`
	display: flex;
	align-items: center;
		...
		return (
			<div>
				<div
					...
					onDrop={onDrop}>
					{/* Add Context button inside text area at bottom left */}
					<div className="absolute bottom-4 left-5 z-10 flex flex-row gap-2">
						<Tooltip>
								...
								<ModelContainer ref={modelSelectorRef}>
									<ModelButtonWrapper ref={buttonRef}>
										<VSCodeButton
											...
											className="p-0 m-0 flex items-center bg-input-background rounded-xs border border-[rgba(255,255,255,0.2)]"
											onClick={handleModelButtonClick}
											style={{ borderRadius: "6px" }}>
											<ButtonContainer>
												{/* Use a proper icon - you can replace this with any Lucide icon */}
												<span className="ml-1 text-sm">Model</span>
												<ChevronDownIcon className="ml-1" size={12} />
											</ButtonContainer>
										</VSCodeButton>
									</ModelButtonWrapper>
									{showModelSelector && (
						...
						<ModeDropdownContainer ref={modeDropdownRef}>
							<ModeDropdownButton isActive={true} onClick={() => setShowModeDropdown(!showModeDropdown)}>
								<span>{mode === "plan" ? "Plan" : "Act"}</span>
								{showModeDropdown ? <ChevronUpIcon size={10} /> : <ChevronDownIcon size={10} />}
							</ModeDropdownButton>
							{showModeDropdown && (
								<ModeDropdownMenu>
									<ModeDropdownItem
					...
					<div
						...
						style={{
							...
							padding: `${9 + thumbnailsHeight}px ${dictationSettings?.dictationEnabled ? "48" : "28"}px 9px 9px`,
						}}
					/>
					<DynamicTextArea
						autoFocus={true}
						...
						style={{
							...
							borderLeft: 0,
							borderRight: 0,
							borderTop: `${thumbnailsHeight}px solid transparent`,
							borderBottom: 0,
					...
					<div
						...
						<div className="flex flex-row items-center gap-2">
							{/* Add Files & Images Button moved to right side */}
							<Tooltip>
