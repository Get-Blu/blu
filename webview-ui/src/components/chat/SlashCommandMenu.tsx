...
const SlashCommandMenu: React.FC<SlashCommandMenuProps> = ({
	...
	const renderCommandSection = (commands: SlashCommand[], title: string, indexOffset: number, showDescriptions: boolean) => {
		...
		return (
			<>
				<div className="text-[11px] text-(--vscode-descriptionForeground) px-3 py-1.5 font-semibold border-b border-(--vscode-editorGroup-border) uppercase tracking-wider flex items-center gap-1.5">
					<i className={`codicon ${title.toLowerCase().includes("workflow") ? "codicon-workflow" : "codicon-terminal"}`} style={{ fontSize: "12px" }} />
					{title}
				</div>
				{commands.map((command, index) => {
					...
					return (
						<div
							className={`slash-command-menu-item py-1.5 px-3 mx-1.5 my-0.5 rounded-[8px] cursor-pointer flex flex-col ${
							...
							<div className="font-medium text-[12px] whitespace-nowrap overflow-hidden text-ellipsis">
								<span className="ph-no-capture">/{command.name}</span>
							</div>
							{showDescriptions && command.description && (
								<div className="text-[11px] opacity-70 whitespace-normal overflow-hidden text-ellipsis">
	...
	return (
		<div
			className="absolute bottom-[calc(100%-12px)] left-[15px] right-[15px] overflow-x-hidden z-1000"
			data-testid="slash-commands-menu"
			onMouseDown={onMouseDown}>
			<div
				className="bg-(--vscode-dropdown-background) border border-(--vscode-editorGroup-border) rounded-[8px] shadow-[0_4px_12px_rgba(0,0,0,0.2)] flex flex-col overflow-y-auto"
				ref={menuRef}
				style={{ maxHeight: "min(280px, calc(50vh))", overscrollBehavior: "contain", paddingBottom: "4px" }}>
				{filteredCommands.length > 0 ? (
					...
					<div className="py-2 px-3 cursor-default flex flex-col">
						<div className="text-[13px] text-(--vscode-descriptionForeground)">No matching commands found</div>
					</div>
				)}
			</div>
		</div>
