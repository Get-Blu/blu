...
const TaskHeader: React.FC<TaskHeaderProps> = ({
	...
	return (
		<div className={"p-2 flex flex-col gap-1.5"}>
			...
			<div
				className={cn(
					"relative overflow-hidden cursor-pointer rounded-lg flex flex-col gap-1.5 z-10 pt-2 pb-2 px-2 hover:opacity-100 bg-(--vscode-toolbar-hoverBackground)/65",
					{
				...
				{isTaskExpanded && (
					<div className="flex flex-col break-words" key={`task-details-${currentTaskItem?.id}`}>
						...
						<TaskTimeline messages={bluMessages} onBlockClick={onScrollToMessage} />
					</div>
				)}
			</div>
