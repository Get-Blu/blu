...
import { AskResponseRequest, NewTaskRequest } from "@shared/proto/blu/task"
 ...
 */
export function useMessageHandlers(messages: BluMessage[], chatState: ChatState): MessageHandlers {
		...
		async (text: string, images: string[], files: string[]) => {
			...
			if (hasContent) {
				...
				if (messages.length === 0) {
					...
					if (bluAsk === "resume_task" || bluAsk === "resume_completed_task") {
					...
					// No bluAsk set - check if task is actively running
