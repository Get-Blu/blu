...
const BluRulesToggleModal: React.FC = () => {
	const {
		...
		setGlobalBluRulesToggles,
		setLocalBluRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalBluRulesToggles?.toggles) {
						setGlobalBluRulesToggles(response.globalBluRulesToggles.toggles)
					}
					if (response.localBluRulesToggles?.toggles) {
						setLocalBluRulesToggles(response.localBluRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalBluRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localBluRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleBluRule(
			ToggleBluRuleRequest.create({
				isGlobal,
				rulePath,
				enabled,
			}),
			...
			.catch((error) => {
				console.error("Error toggling Blu rule:", error)
			})
	}
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Blu Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Blu Rules & Workflows" : "Show Blu Rules & Workflows"}
			...
			{isVisible && (
				<div
					...
					<div className="text-xs text-(--vscode-descriptionForeground) mb-4">
						{currentView === "rules" ? (
							<p>
								Rules allow you to provide Blu with system-level guidance. Think of them as a persistent way to
								include context and preferences for your projects or globally for every conversation.{" "}
								<VSCodeLink
									className="text-xs"
									href="https://docs.getblu.in/features/blu-rules"
							...
							<p>
								Workflows allow you to define a series of steps to guide Blu through a repetitive set of tasks,
								...
								<VSCodeLink
									className="text-xs"
									href="https://docs.getblu.in/features/slash-commands/workflows"
									style={{ display: "inline" }}>
									Docs
								</VSCodeLink>
							</p>
...
export default BluRulesToggleModal
