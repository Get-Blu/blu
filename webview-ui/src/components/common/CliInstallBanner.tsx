...
import { EmptyRequest, Int64Request } from "@shared/proto/index.blu"
...
export const CliInstallBanner: React.FC = () => {
	...
	const [isBluCliInstalled, setIsBluCliInstalled] = useState(false)
	...
	useEffect(() => {
		const checkInstallation = async () => {
			try {
				...
				setIsBluCliInstalled(result.value)
			} catch (error) {
	...
	const handleInstallClick = async () => {
		if (!isBluCliInstalled) {
			try {
				// Call the backend to initiate CLI installation
				await StateServiceClient.installBluCli(EmptyRequest.create())
	...
	const handleCopyCommand = async (e: React.MouseEvent) => {
		...
		await navigator.clipboard.writeText("npm install -g blu")
	...
	return (
		<div
			...
			<h4 className="m-0 flex items-center gap-2" style={{ paddingRight: "24px" }}>
				...
				{isMacOSOrLinux() ? "Blu for CLI is here!" : "Blu CLI Information"}
			</h4>
			<p className="m-0">
				{isMacOSOrLinux() ? (
					<>
						Install to use Blu directly in your terminal and enable subagent capabilities. Blu can spawn{" "}
						<code>blu</code> commands to handle focused tasks like exploring large codebases for information. This
						keeps your main context window clean by running these operations in separate subprocesses.{" "}
						<a
							href="https://docs.getblu.in/blu-cli/overview"
					...
					<>
						Blu CLI is available for macOS and Linux! Coming <code>soon</code> to other platforms.{" "}
						<a
							href="https://docs.getblu.in/blu-cli/overview"
							rel="noopener noreferrer"
							style={{ color: "var(--vscode-textLink-foreground)" }}
				...
				{isMacOSOrLinux() ? (
					...
					<div className="flex gap-2">
						...
						<VSCodeButton
							...
							className="flex-1"
							disabled
							title="Blu CLI & subagents are only available on macOS & Linux">
