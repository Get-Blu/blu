...
import { BluAuthContext, BluAuthContextType, BluAuthProvider, useBluAuth } from "@/context/BluAuthContext"
import {
...
const BluAuthProviderWithOverrides: React.FC<{
	overrides?: Partial<BluAuthContextType>
	children: React.ReactNode
}> = ({ overrides, children }) => {
	const authContext = useBluAuth()
	return <BluAuthContext.Provider value={{ ...authContext, ...overrides }}>{children}</BluAuthContext.Provider>
}

export const createStorybookDecorator =
	(overrideStates?: Partial<ExtensionStateContextType>, classNames?: string, authOverrides?: Partial<BluAuthContextType>) =>
	(Story: any) => (
		<ExtensionStateProviderWithOverrides overrides={overrideStates}>
			<BluAuthProviderWithOverrides overrides={authOverrides}>
				<div className={cn("max-w-lg mx-auto", classNames)}>
					<Story />
				</div>
			</BluAuthProviderWithOverrides>
