...
`
	...
	beforeEach(async () => {
		...
		// Create .blurules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".blurules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.blurules/hooks/
	...
	describe("Error Handling", () => {
		...
		it("should update message state on hook failure", async function () {
			...
			const mockHandler = {
				...testHandler,
				getBluMessages: () => messages,
				addToBluMessages: async (msg: BluMessage) => {
					messages.push(msg)
				},
				updateBluMessage: async (index: number, updates: Partial<BluMessage>) => {
					if (messages[index]) {
