...
describe("Blu Extension", () => {
	...
	it("should verify extension ID matches package.json", async () => {
		...
		const bluExtensionApi = vscode.extensions.getExtension(id)

		bluExtensionApi?.id.should.equal(id)
	})

	it("should successfully execute the plus button command", async () => {
		await new Promise((resolve) => setTimeout(resolve, 400))
		await vscode.commands.executeCommand("blu.plusButtonClicked")
	})
