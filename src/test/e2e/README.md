...
This directory contains the end-to-end tests for the Blu VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing Blu's backend interactions
...
- **`sidebar`** - Playwright Frame object for the Blu extension's sidebar
...
await addSelectedCodeToBluWebview(page)
```
...
2. **Playwright will open:**
   - A VS Code window with Blu extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with Blu loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for Blu API calls
