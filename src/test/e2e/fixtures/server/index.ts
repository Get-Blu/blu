...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/BluAccount"
import { E2E_MOCK_API_RESPONSES, E2E_REGISTERED_MOCK_ENDPOINTS } from "./api"
import { BluDataMock } from "./data"
...
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[BluApiServerMock]", ...args)
	}
}

export class BluApiServerMock {
	static globalSharedServer: BluApiServerMock | null = null
	...
	public readonly API_USER = new BluDataMock("personal")
	...
	public static async startGlobalServer(): Promise<BluApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (BluApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return BluApiServerMock.globalSharedServer
		}
		...
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (isAuthRequired && authToken) {
				...
				const user = BluApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				BluApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = BluApiServerMock.matchRoute(path, method)
				...
				const controller = BluApiServerMock.globalSharedServer!
				...
				if (baseRoute === "/api/v1") {
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching BluAuthProvider expectations
						return sendApiResponse({
					...
					if (endpoint === "/chat/completions" && method === "POST") {
						...
						if (body.includes("[diff.test.ts] Hello, Blu!")) {
								...
								choices: [
									{
										...
										message: {
											role: "assistant",
											content: "Hello! I'm a mock Blu API response.",
										},
										finish_reason: "stop",
									},
								],
		...
		const controller = new BluApiServerMock(server)
		BluApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			BluApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				BluApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`BluApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = BluApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		BluApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		BluApiServerMock.globalSockets.clear()
		...
		BluApiServerMock.globalSharedServer = null
	}
}
