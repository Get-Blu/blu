...
e2e("Views - can set up API keys and navigate to Settings from Chat", async ({ sidebar }) => {
	...
	// Test Blu provider option
	await providerSelectorInput.click({ delay: 100 })
	// Wait for dropdown to appear and find Blu option
	await expect(sidebar.getByTestId("provider-option-blu")).toBeVisible()
	await sidebar.getByTestId("provider-option-blu").click({ delay: 100 })
	await expect(sidebar.getByRole("button", { name: "Sign Up with Blu" })).toBeVisible()
	...
	// blu logo has the name "clline-logo"
	const bluLogo = sidebar.locator(".size-20 > path")
	await expect(bluLogo).toBeVisible()
