...
import { BluApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public static async openBluSidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /Blu/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the Blu extension.
 *
 * Extended Playwright test configuration for Blu E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the Blu VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared BluApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with Blu sidebar opened
 * - `sidebar`: Playwright Frame object representing the Blu extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a BluApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and Blu sidebar open
 * - **sidebar**: Provides access to the Blu extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the Blu extension in development mode
...
export const e2e = test
	.extend<{ server: BluApiServerMock | null }>({
		server: async ({}, use) => {
			// Start server if it doesn't exist
			if (!BluApiServerMock.globalSharedServer) {
				await BluApiServerMock.startGlobalServer()
			}
			await use(BluApiServerMock.globalSharedServer)
		},
	})
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openBluSidebar(page)
