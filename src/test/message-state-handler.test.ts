 ...
 * to prevent race conditions, particularly the TOCTOU bug in addToBluMessages
 */
describe("MessageStateHandler Mutex Protection", () => {
	...
	/**
	 * Helper to create a test BluMessage
	 */
	function createTestMessage(text: string): BluMessage {
		return {
			ts: Date.now(),
			type: "say",
			say: "text",
	...
	it("should initialize with empty message arrays", () => {
		...
		handler.getBluMessages().length.should.equal(0)
	...
	it("should set and get blu messages", () => {
		const handler = createTestHandler()
		const testMessages = [createTestMessage("test1"), createTestMessage("test2")]

		handler.setBluMessages(testMessages)
		handler.getBluMessages().should.deepEqual(testMessages)
	})

	/**
	 * CRITICAL TEST: Verify that addToBluMessages is atomic
	 * This test simulates the race condition that can occur when multiple
	 * addToBluMessages calls happen concurrently without proper mutex protection
	 */
	it("should handle concurrent addToBluMessages atomically", async function () {
		...
		await handler.addToBluMessages(initialMsg)

		// Verify initial state
		const messages = handler.getBluMessages()
		...
		const results = await Promise.all([
			handler.addToBluMessages(msg1),
			handler.addToBluMessages(msg2),
			handler.addToBluMessages(msg3),
		])
		...
		// Get final state
		const finalMessages = handler.getBluMessages()
	...
	/**
	 * Test that updateBluMessage operations are atomic
	 */
	it("should handle concurrent updateBluMessage atomically", async function () {
		this.timeout(5000)
		...
		handler.setBluMessages(msgs)

		// Perform concurrent updates to different messages
		await Promise.all([
			handler.updateBluMessage(0, { text: "updated1" }),
			handler.updateBluMessage(1, { text: "updated2" }),
			handler.updateBluMessage(2, { text: "updated3" }),
		])
	...
	/**
	 * Test that deleteBluMessage operations are atomic
	 */
	it("should handle deleteBluMessage with proper validation", async () => {
		const handler = createTestHandler()
		...
		await handler.deleteBluMessage(1)
	 ...
	 */
	it("should throw error for invalid message index in updateBluMessage", async () => {
		const handler = createTestHandler()
		handler.setBluMessages([createTestMessage("msg1")])

		try {
			await handler.updateBluMessage(5, { text: "invalid" })
	...
	/**
	 * Test that invalid indices are rejected in deleteBluMessage
	 */
	it("should throw error for invalid message index in deleteBluMessage", async () => {
		const handler = createTestHandler()
		handler.setBluMessages([createTestMessage("msg1")])
	 ...
	 */
	it("should handle overwriteBluMessages atomically", async () => {
		const handler = createTestHandler()

		// Set initial messages
		handler.setBluMessages([createTestMessage("old1"), createTestMessage("old2")])

		// Overwrite with new messages
		const newMessages = [createTestMessage("new1"), createTestMessage("new2"), createTestMessage("new3")]
		await handler.overwriteBluMessages(newMessages)
