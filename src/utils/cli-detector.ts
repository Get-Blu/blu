...
/**
 * Check if the Blu CLI tool is installed on the system
 * @returns true if CLI is installed, false otherwise
 */
export async function isBluCliInstalled(): Promise<boolean> {
	try {
		// Try to get the version of the blu CLI tool
		// This will fail if the tool is not installed
		const { stdout } = await execAsync("blu version", {
			timeout: 5000, // 5 second timeout
		})

		// If we get here, the CLI is installed
		// We could also validate the version if needed
		return stdout.includes("Blu CLI Version") || stdout.includes("Blu Core Version")
	} catch (error) {
...
/**
 * Detect if the current Blu instance is running as a CLI subagent.
 * CLI subagents are identified by specific parameter patterns set by the transformBluCommand function.
 *     TODO - For now we are relying on the maxConsecutiveMistakes value, which will only ever be "3"
 *     unless users pass in "-s max_consecutive_mistakes=6" via Blu CLI. Would like better detection.
