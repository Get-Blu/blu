...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "blu-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .blurules/workflows directory specifically", async () => {
		// Create a test directory structure
		const blurulesDirTest = path.join(tmpDir, "blurules-test")
		const blurulesDirPath = path.join(blurulesDirTest, ".blurules")

		// Create .blurules directory and root files
		await fs.mkdir(blurulesDirPath, { recursive: true })
		await fs.writeFile(path.join(blurulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(blurulesDirPath, "settings.js"), "// settings")

		// Create .blurules/other directory and files
		const otherDirPath = path.join(blurulesDirPath, "other")
		...
		// Create .blurules/workflows directory and files
		const workflowsDirPath = path.join(blurulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(blurulesDirPath)
		...
		const filteredFiles = await readDirectory(blurulesDirPath, [[".blurules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(blurulesDirPath, "config.json"),
			path.resolve(blurulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(blurulesDirPath, [
			[".blurules", "workflows"],
			[".blurules", "other"],
		])
		...
		const rootOnlyFiles = [path.resolve(blurulesDirPath, "config.json"), path.resolve(blurulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .blurules/hooks directory specifically", async () => {
		// Create a test directory structure
		const blurulesDirTest = path.join(tmpDir, "blurules-hooks-test")
		const blurulesDirPath = path.join(blurulesDirTest, ".blurules")
		...
		// Create .blurules/hooks directory and files
		const hooksDirPath = path.join(blurulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(blurulesDirPath, [[".blurules", "hooks"]])
