...
export function isNextGenModelProvider(providerInfo: ApiProviderInfo): boolean {
	...
	return ["blu", "anthropic", "gemini", "openrouter", "xai", "openai", "minimax", "openai-native", "vercel-ai-gateway"].some(
		(id) => providerId === id,
	)
}
...
export function isGemini3PlusModelFamily(id: string): boolean {
	const modelId = normalize(id)
...
export function isNextGenModelFamily(id: string): boolean {
	...
	return (
		...
		isGemini3PlusModelFamily(modelId) ||
