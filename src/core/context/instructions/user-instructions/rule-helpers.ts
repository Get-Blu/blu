...
export async function synchronizeRuleToggles(
	...
	currentToggles: BluRulesToggles,
	allowedFileExtension: string = "",
	excludedPaths: string[][] = [],
): Promise<BluRulesToggles> {
 ...
 */
export function combineRuleToggles(toggles1: BluRulesToggles, toggles2: BluRulesToggles): BluRulesToggles {
 ...
 */
export const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: BluRulesToggles) => {
...
/**
 * Handles converting any directory into a file (specifically used for .blurules and .blurules/workflows)
 * The old .blurules file or .blurules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
export async function ensureLocalBluDirExists(blurulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(blurulePath)

		if (exists && !(await isDirectory(blurulePath))) {
			// logic to convert .blurules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(blurulePath, "utf8")
			const tempPath = blurulePath + ".bak"
			await fs.rename(blurulePath, tempPath) // create backup
			try {
				await fs.mkdir(blurulePath, { recursive: true })
				await fs.writeFile(path.join(blurulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(blurulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, blurulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const globalBluWorkflowFilePath = await ensureWorkflowsDirectoryExists()
				filePath = path.join(globalBluWorkflowFilePath, filename)
			} else {
				const globalBluRulesFilePath = await ensureRulesDirectoryExists()
				filePath = path.join(globalBluRulesFilePath, filename)
			}
		} else {
			const localBluRulesFilePath = path.resolve(cwd, GlobalFileNames.bluRules)

			const hasError = await ensureLocalBluDirExists(localBluRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localBluRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, GlobalFileNames.workflows)

				const hasError = await ensureLocalBluDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// blurules file creation
				filePath = path.join(localBluRulesFilePath, filename)
			}
		}
