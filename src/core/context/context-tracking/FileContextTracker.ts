...
import type { BluMessage } from "@shared/ExtensionMessage"
...
// If a user modifies a file outside of Blu, the context may become stale and need to be updated.
// We do not want Blu to reload the context every time a file is modified, so we use this class merely
// to inform Blu that the change has occurred, and tell Blu to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Blu was unable to complete a diff edit.
// a diff edit because the file was modified since Blu last read it.

// FileContextTracker
/**
This class is responsible for tracking file operations.
If the full contents of a file are passed to Blu via a tool, mention, or edit, the file is marked as active.
If a file is modified outside of Blu, we detect and track this change to prevent stale context.
...
export class FileContextTracker {
	...
	private recentlyEditedByBlu = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.on("change", () => {
			if (this.recentlyEditedByBlu.has(filePath)) {
				this.recentlyEditedByBlu.delete(filePath) // This was an edit by Blu, no need to inform Blu
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Blu
	 ...
	 * This is the main entry point for FileContextTracker and is called when a file is passed to Blu via a tool, mention, or edit.
	 */
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "blu_edited" | "file_mentioned") {
		try {
			const cwd = await getCwd()
			if (!cwd) {
				console.info("No workspace folder available - cannot determine current working directory")
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: FileMetadataEntry["record_source"]) {
		try {
			...
			const newEntry: FileMetadataEntry = {
				...
				blu_read_date: getLatestDateForField(filePath, "blu_read_date"),
				blu_edit_date: getLatestDateForField(filePath, "blu_edit_date"),
			...
			switch (source) {
				...
				// blu_edited: Blu has edited the file
				case "blu_edited":
					newEntry.blu_read_date = now
					newEntry.blu_edit_date = now
					break

				// read_tool/file_mentioned: Blu has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.blu_read_date = now
					break
	...
	/**
	 * Marks a file as edited by Blu to prevent false positives in file watchers
	 */
	markFileAsEditedByBlu(filePath: string): void {
		this.recentlyEditedByBlu.add(filePath)
	}
	...
	/**
	 * Detects files that were edited by Blu or users after a specific message timestamp
	 * This is used when restoring checkpoints to warn about potential file content mismatches
	 */
	async detectFilesEditedAfterMessage(messageTs: number, deletedMessages: BluMessage[]): Promise<string[]> {
		const editedFiles: string[] = []

		try {
			// Check task metadata for files that were edited by Blu or users after the message timestamp
			...
			if (taskMetadata?.files_in_context) {
				for (const fileEntry of taskMetadata.files_in_context) {
					const bluEditedAfter = fileEntry.blu_edit_date && fileEntry.blu_edit_date > messageTs
					const userEditedAfter = fileEntry.user_edit_date && fileEntry.user_edit_date > messageTs

					if (bluEditedAfter || userEditedAfter) {
