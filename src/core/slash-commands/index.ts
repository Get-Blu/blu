...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsBlurulesFileCheck: boolean }> {
	...
	for (const { tag, regex } of tagPatterns) {
		...
		if (match) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsBlurulesFileCheck: commandName === "newrule" }
			}
			...
			if (matchingWorkflow) {
				try {
					...
					return { processedText, needsBlurulesFileCheck: false }
				} catch (error) {
	...
	return { processedText: text, needsBlurulesFileCheck: false }
}
