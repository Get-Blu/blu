...
export class ToolUseHandler {
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of this.pendingToolUses.values()) {
			...
			if (pending.name.includes(BLU_MCP_TOOL_IDENTIFIER)) {
				const [key, toolName] = pending.name.split(BLU_MCP_TOOL_IDENTIFIER)
				results.push({
					type: "tool_use",
					name: BluDefaultTool.MCP_USE,
					params: {
				...
				results.push({
					type: "tool_use",
					name: pending.name as BluDefaultTool,
