...
├── registry/
│   ├── BluToolSet.ts            # Tool set management & registry
...
interface PromptVariant {
  ...
  // Tool configuration
  tools?: BluDefaultTool[];                    // Ordered list of tools to include
  toolOverrides?: { [K in BluDefaultTool]?: ConfigOverride }; // Tool-specific customizations
}
...
```markdown
You are Blu, a highly skilled software engineer...
...
Tools are managed through the `BluToolSet` and can be configured per variant:

```typescript
class BluToolSet {
  private static variants: Map<ModelFamily, Set<BluToolSet>> = new Map();

  static register(config: BluToolSpec): BluToolSet {
    return new BluToolSet(config.id, config);
  }

  static getTools(variant: ModelFamily): BluToolSet[] {
...
public static async getToolsPrompts(variant: PromptVariant, context: SystemPromptContext) {
  const tools = BluToolSet.getTools(variant.family);
...
export const config = createVariant(ModelFamily.NEXT_GEN)
  ...
  .tools(
    ...
    BluDefaultTool.LIST_CODE_DEF,
    BluDefaultTool.BROWSER,
    BluDefaultTool.WEB_FETCH,  // Additional tool for next-gen
    BluDefaultTool.MCP_USE,
    BluDefaultTool.MCP_ACCESS,
    BluDefaultTool.ASK,
    BluDefaultTool.ATTEMPT,
...
const config = createVariant(ModelFamily.GENERIC)
  ...
  .overrideTool(BluDefaultTool.BASH, {          // Optional, tool overrides
    enabled: false,
  })
...
The system supports the following tools (mapped to `BluDefaultTool` enum):
...
Each tool in Blu follows a specific structure with variants for different model families. Here's the anatomy of a tool:
...
const id = BluDefaultTool.MY_NEW_TOOL // Add to enum first
...
const generic: BluToolSpec = {
	...
	parameters: [
		...
		},
		{
			...
			dependencies: [BluDefaultTool.SOME_OTHER_TOOL], // Only show if dependency exists
		},
	],
}
...
First, add your tool ID to the `BluDefaultTool` enum:

```typescript
// src/shared/tools.ts
export enum BluDefaultTool {
	// ... existing tools
...
const id = BluDefaultTool.MY_NEW_TOOL
...
export function registerBluToolSets(): void {
...
```typescript
// In your tool handler class (e.g., BluProvider)
...
```typescript
const contextAwareTool: BluToolSpec = {
	variant: ModelFamily.GENERIC,
	id: BluDefaultTool.CONTEXT_TOOL,
	name: "context_tool",
...
```typescript
const claude: BluToolSpec = {
	variant: ModelFamily.GENERIC,
	id: BluDefaultTool.MODEL_SPECIFIC_TOOL,
...
```typescript
const dependentTool: BluToolSpec = {
	variant: ModelFamily.GENERIC,
	id: BluDefaultTool.DEPENDENT_TOOL,
	name: "dependent_tool",
	...
	parameters: [
		...
		},
		{
			...
			dependencies: [BluDefaultTool.TODO],
		},
	],
}
```
...
const id = BluDefaultTool.ANALYZE_FILE
...
This comprehensive guide should help developers understand both the architecture and practical steps needed to extend Blu with new tools.
