...
This directory contains the tool registration system for Blu tools. The system automatically collects and registers all tool variants with the `BluToolSet` provider.

## Overview

Each tool file in this directory exports a `{toolName}_variants` array containing tool specifications for different prompt variants (e.g., Claude, GPT). The registration system automatically imports all these variants and registers them with the `BluToolSet` provider.
...
```typescript
import { BluToolSet } from "../registry/BluToolSet";
import { PromptVariant } from "@/shared/tools";

// Get all tools for a specific variant
const claudeTools = BluToolSet.getTools(PromptVariant.CLAUDE);

// Get a specific tool by name
const writeToFileTool = BluToolSet.getToolByName("write_to_file", PromptVariant.CLAUDE);
```
...
```typescript
import { BluDefaultTool, PromptVariant, type BluToolSpec } from "@/shared/tools";
