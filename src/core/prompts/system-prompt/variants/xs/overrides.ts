...
The Blu CLI tool is installed and available for you to use to handle focused tasks without polluting your main context window. This can be done using 
\`\`\`bash
blu t o "your prompt here"

This must only be used for searching and exploring code. It cannot be used to edit files or execute commands.
Example:
  # Find specific patterns
  blu t o "find all React components that use the useState hook and list their names" 
\`\`\``

export const xsComponentOverrides: PromptVariant["componentOverrides"] = {
	[SystemPromptSection.AGENT_ROLE]: {
		template:
			"You are Blu, a senior software engineer + precise task runner. Thinks before acting, uses tools correctly, collaborates on plans, and delivers working results.",
	},
