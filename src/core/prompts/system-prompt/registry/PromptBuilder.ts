...
export class PromptBuilder {
	...
	private static getEnabledTools(variant: PromptVariant, context: SystemPromptContext) {
		let resolvedTools: ReturnType<typeof BluToolSet.getTools> = []
		...
		if (variant?.tools?.length) {
			...
			resolvedTools = BluToolSet.getToolsForVariantWithFallback(variant.family, requestedIds)
			...
			resolvedTools = BluToolSet.getTools(variant.family)
	...
	public static tool(config: BluToolSpec, registry: BluDefaultTool[], context: SystemPromptContext): string {
