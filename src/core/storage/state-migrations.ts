...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			console.log("Migrating custom instructions to global Blu rules...")

			// Create global .blurules directory if it doesn't exist
			...
			console.log("Successfully migrated custom instructions to global Blu rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const bluAccountId = await context.secrets.get("bluAccountId")
