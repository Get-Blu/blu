...
export const GlobalFileNames = {
	...
	mcpSettings: "blu_mcp_settings.json",
	bluRules: ".blurules",
	workflows: ".blurules/workflows",
	hooksDir: ".blurules/hooks",
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const bluRulesDir = path.join(userDocumentsPath, "Blu", "Rules")
	try {
		await fs.mkdir(bluRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Blu", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return bluRulesDir
}
...
export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const bluWorkflowsDir = path.join(userDocumentsPath, "Blu", "Workflows")
	try {
		await fs.mkdir(bluWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Blu", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return bluWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Blu", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Blu", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const bluHooksDir = path.join(userDocumentsPath, "Blu", "Hooks")
	try {
		await fs.mkdir(bluHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Blu", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return bluHooksDir
}
...
export async function getSavedBluMessages(taskId: string): Promise<BluMessage[]> {
...
export async function saveBluMessages(taskId: string, uiMessages: BluMessage[]) {
	try {
 ...
 * 2. Each workspace root's .blurules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .blurules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .blurules/hooks folder in this workspace root.
