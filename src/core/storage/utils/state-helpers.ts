...
export async function readSecretsFromDisk(context: ExtensionContext): Promise<Secrets> {
	const [
		...
		context.secrets.get("bluAccountId") as Promise<Secrets["bluAccountId"]>,
		context.secrets.get("blu:bluAccountId") as Promise<Secrets["blu:bluAccountId"]>,
	...
	return {
		...
		apiKey,
		openRouterApiKey,
		bluAccountId: firebaseBluAccountId,
		"blu:bluAccountId": bluAccountId,
		huggingFaceApiKey,
		huaweiCloudMaasApiKey,
		basetenApiKey,
		zaiApiKey,
...
export async function readWorkspaceStateFromDisk(context: ExtensionContext): Promise<LocalState> {
	const localBluRulesToggles = context.workspaceState.get("localBluRulesToggles") as BluRulesToggles | undefined
	const localWindsurfRulesToggles = context.workspaceState.get("localWindsurfRulesToggles") as BluRulesToggles | undefined
	const localCursorRulesToggles = context.workspaceState.get("localCursorRulesToggles") as BluRulesToggles | undefined
	const localWorkflowToggles = context.workspaceState.get("workflowToggles") as BluRulesToggles | undefined
...
export async function readGlobalStateFromDisk(context: ExtensionContext): Promise<GlobalStateAndSettings> {
	try {
		...
		const globalBluRulesToggles =
			context.globalState.get<GlobalStateAndSettings["globalBluRulesToggles"]>("globalBluRulesToggles")
		...
		const promptEngineeringSettings =
			context.globalState.get<GlobalStateAndSettings["promptEngineeringSettings"]>("promptEngineeringSettings")
		 ...
		 * The primary root is the main workspace folder that Blu focuses on when dealing with
		...
		return {
			...
			promptEngineeringSettings: { ...DEFAULT_PROMPT_ENGINEERING_SETTINGS, ...promptEngineeringSettings },
