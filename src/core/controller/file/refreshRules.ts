import { refreshBluRulesToggles } from "@core/context/instructions/user-instructions/blu-rules"
import { refreshExternalRulesToggles } from "@core/context/instructions/user-instructions/external-rules"
import { refreshWorkflowToggles } from "@core/context/instructions/user-instructions/workflows"
import { EmptyRequest } from "@shared/proto/blu/common"
import { RefreshedRules } from "@shared/proto/blu/file"
...
export async function refreshRules(controller: Controller, _request: EmptyRequest): Promise<RefreshedRules> {
	try {
		...
		const { globalToggles, localToggles } = await refreshBluRulesToggles(controller, cwd)
