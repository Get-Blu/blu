...
import { Empty, StringRequest } from "../../../shared/proto/blu/common"
...
export async function openFocusChainFile(controller: Controller, request: StringRequest): Promise<Empty> {
	...
	if (currentTask) {
		...
		// TODO - can we decouple this from BluMessages?
		const bluMessages = currentTask.messageStateHandler.getBluMessages()
		const lastProgressMessage = bluMessages
			.slice()
			.reverse()
