...
import { appendBluStealthModels } from "./models/refreshOpenRouterModels"
...
export class Controller {
	...
	constructor(readonly context: vscode.ExtensionContext) {
		...
		HostProvider.get().logToChannel("BluProvider instantiated")
	...
	*/
	...
	async cancelTask() {
		...
		try {
			...
			if (this.task) {
				// 'abandoned' will prevent this blu instance from affecting future blu instance gui. this may happen if its hanging on a streaming request
	...
	private async fetchMcpMarketplaceFromApi(): Promise<McpMarketplaceCatalog> {
		const response = await axios.get(`${BluEnv.config().mcpBaseUrl}/marketplace`, {
			headers: {
				"Content-Type": "application/json",
				"User-Agent": "blu-vscode-extension",
			},
		})
	...
	async readOpenRouterModels(): Promise<Record<string, ModelInfo> | undefined> {
		...
		try {
			if (await fileExistsAtPath(openRouterModelsFilePath)) {
				...
				return appendBluStealthModels(models)
			}
		} catch (error) {
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const promptEngineeringSettings = this.stateManager.getGlobalSettingsKey("promptEngineeringSettings")
		console.log("[Backend Debug] getStateToPostToWebview: promptEngineeringSettings =", promptEngineeringSettings)
		...
		const globalBluRulesToggles = this.stateManager.getGlobalSettingsKey("globalBluRulesToggles")
		...
		const localBluRulesToggles = this.stateManager.getWorkspaceStateKey("localBluRulesToggles")
		...
		const bluMessages = this.task?.messageStateHandler.getBluMessages() || []
		...
		const environment = BluEnv.config().environment
		...
		return {
			...
			environment,
			distinctId,
			globalBluRulesToggles: globalBluRulesToggles || {},
			localBluRulesToggles: localBluRulesToggles || {},
	...
	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of blu messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between BluProvider instances since there could be multiple instances of the extension running at once. For example when we cached blu messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
	- Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

	We need to use a unique identifier for each BluProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
