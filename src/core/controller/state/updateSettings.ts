...
} from "@shared/proto/blu/state"
import { convertProtoToApiProvider } from "@shared/proto-conversions/models/api-configuration-conversion"
import { OpenaiReasoningEffort } from "@shared/storage/types"
import { TelemetrySetting } from "@shared/TelemetrySetting"
import { BluEnv } from "@/config"
...
export async function updateSettings(controller: Controller, request: UpdateSettingsRequest): Promise<Empty> {
	try {
		if (request.bluEnv !== undefined) {
			BluEnv.setEnvironment(request.bluEnv)
		...
		if (request.promptEngineeringSettings !== undefined) {
			console.log("[Backend Debug] === PROMPT ENGINEERING SETTINGS UPDATE ===")
			console.log("[Backend Debug] Received promptEngineeringSettings:", request.promptEngineeringSettings)

			// Convert from protobuf format (snake_case) to TypeScript format (camelCase)
			const currentSettings = controller.stateManager.getGlobalSettingsKey("promptEngineeringSettings")
			console.log("[Backend Debug] Current settings from state:", currentSettings)
			console.log("[Backend Debug] Current settings enabled:", currentSettings?.enabled)
			console.log("[Backend Debug] Current settings enhancePromptEnabled:", currentSettings?.enhancePromptEnabled)
			console.log("[Backend Debug] New settings enabled:", request.promptEngineeringSettings.enabled)
			console.log(
				"[Backend Debug] New settings enhancePromptEnabled:",
				request.promptEngineeringSettings.enhancePromptEnabled,
			)

			const promptEngineeringSettings = {
				enabled: request.promptEngineeringSettings.enabled ?? currentSettings?.enabled ?? false,
				enhancePromptEnabled:
					request.promptEngineeringSettings.enhancePromptEnabled ?? currentSettings?.enhancePromptEnabled ?? false,
			}

			console.log("[Backend Debug] New promptEngineeringSettings to set:", promptEngineeringSettings)
			console.log("[Backend Debug] New enabled value:", promptEngineeringSettings.enabled)
			console.log("[Backend Debug] New enhancePromptEnabled value:", promptEngineeringSettings.enhancePromptEnabled)

			controller.stateManager.setGlobalState("promptEngineeringSettings", promptEngineeringSettings)
			console.log("[Backend Debug] Settings updated in global state")

			// Verify the setting was saved
			const savedSettings = controller.stateManager.getGlobalSettingsKey("promptEngineeringSettings")
			console.log("[Backend Debug] Verified saved settings:", savedSettings)

			// Add to log to show up in frontend
			controller.postStateToWebview().then(() => {
				console.log("[Backend Debug] State posted to webview after update")
			})

			console.log("[Backend Debug] === END PROMPT ENGINEERING SETTINGS UPDATE ===")
		}
		...
		if (request.focusChainSettings !== undefined) {
			{
				...
				const focusChainSettings = {
					enabled: isEnabled,
					remindBluInterval: request.focusChainSettings.remindBluInterval,
				}
