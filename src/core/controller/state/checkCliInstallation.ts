import { Boolean } from "@shared/proto/blu/common"
import { isBluCliInstalled } from "@/utils/cli-detector"
import { Controller } from ".."

/**
 * Check if the Blu CLI is installed
...
export async function checkCliInstallation(_controller: Controller): Promise<Boolean> {
	try {
		const isInstalled = await isBluCliInstalled()
