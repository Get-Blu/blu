...
async function handleUnaryRequest(
	...
	try {
		...
		const handler = await getHandler(request.service, request.method)
...
async function getHandler(serviceName: string, methodName: string): Promise<any> {
	// Dynamically import to break circular dependency: Controller -> grpc-handler -> protobus-services -> handlers -> Controller
