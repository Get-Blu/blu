...
export async function refreshOpenRouterModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	try {
			...
			for (const rawModel of rawModels as OpenRouterRawModelInfo[]) {
				...
				switch (rawModel.id) {
					...
					case "anthropic/claude-opus-4":
						modelInfo.supportsPromptCache = true
						// Opus 4.5 pricing is lower than 4.1 ($5/$25)
						if (rawModel.id.includes("4.5")) {
					...
					case "moonshotai/kimi-k2":
						...
						break
					case "openai/gpt-5.2":
					case "openai/gpt-5.2-pro":
					case "openai/gpt-5.2-thinking":
					case "openai/gpt-5.2-mini":
					case "openai/gpt-5.2-instant":
					case "openai/gpt-5":
					...
					case "google/gemini-3-flash-preview":
						modelInfo.contextWindow = 2_000_000
						break
		...
		if (cachedModels) {
			...
			return appendBluStealthModels(cachedModels as Record<string, ModelInfo>)
		}
	}
...
export function appendBluStealthModels(currentModels: Record<string, ModelInfo>): Record<string, ModelInfo> {
