import { RecordingResult } from "@shared/proto/blu/dictation"
...
/**
 * Handles the installation of missing dependencies with Blu
 */
async function handleInstallWithBlu(
...
async function handleMissingDependency(
	...
	const installWithBlu = "Install with Blu"
	...
	const action = await HostProvider.window.showMessage({
		...
		options: { items: [installWithBlu, installManually, dismiss] },
	})

	if (action.selectedOption === installWithBlu) {
		await handleInstallWithBlu(controller, config.dependencyName, config.installCommand, platform)
...
async function handleSignInError(controller: Controller, errorMessage: string): Promise<void> {
	const signInAction = "Sign in to Blu"
	const action = await HostProvider.window.showMessage({
...
export const startRecording = async (controller: Controller): Promise<RecordingResult> => {
	...
	try {
		...
		if (!userInfo?.user?.uid) {
			throw new Error("Please sign in to your Blu Account to use Dictation.")
		}
