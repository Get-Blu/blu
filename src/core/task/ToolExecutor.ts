...
import { BluAsk, BluSay } from "@shared/ExtensionMessage"
...
export class ToolExecutor {
	...
	private shouldAutoApproveTool(toolName: BluDefaultTool): boolean | [boolean, boolean] {
		return this.autoApprover.shouldAutoApproveTool(toolName)
	}

	private async shouldAutoApproveToolWithPath(
		blockname: BluDefaultTool,
	...
	constructor(
		...
		private sayAndCreateMissingParamError: (toolName: BluDefaultTool, paramName: string, relPath?: string) => Promise<any>,
		private removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: BluAsk | BluSay) => Promise<void>,
	...
	private asToolConfig(): TaskConfig {
		const config: TaskConfig = {
			...
			services: {
				...
				bluIgnoreController: this.bluIgnoreController,
	...
	private registerToolHandlers(): void {
		const validator = new ToolValidator(this.bluIgnoreController)
		...
		this.coordinator.register(writeHandler) // registers as "write_to_file" (BluDefaultTool.FILE_NEW)
		this.coordinator.register(new SharedToolHandler(BluDefaultTool.FILE_EDIT, writeHandler))
		this.coordinator.register(new SharedToolHandler(BluDefaultTool.NEW_RULE, writeHandler))
	 ...
	 */
	private static readonly PLAN_MODE_RESTRICTED_TOOLS: BluDefaultTool[] = [
		BluDefaultTool.FILE_NEW,
		BluDefaultTool.FILE_EDIT,
		BluDefaultTool.NEW_RULE,
	]
	 ...
	 */
	private isPlanModeToolRestricted(toolName: BluDefaultTool): boolean {
