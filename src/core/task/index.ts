...
import {
	getGlobalBluRules,
	getLocalBluRules,
	refreshBluRulesToggles,
} from "@core/context/instructions/user-instructions/blu-rules"
import { sendPartialMessageEvent } from "@core/controller/ui/subscribeToPartialMessage"
import { BluIgnoreController } from "@core/ignore/BluIgnoreController"
...
import { BLU_MCP_TOOL_IDENTIFIER } from "@shared/mcp"
import { convertBluMessageToProto } from "@shared/proto-conversions/blu-message"
import { BluDefaultTool } from "@shared/tools"
import { BluAskResponse } from "@shared/WebviewMessage"
...
import { isSubagentCommand, transformBluCommand } from "@/integrations/cli-subagents/subagent_command"
import { ErrorService } from "@/services/error"
import { BluError, BluErrorType } from "@/services/error/BluError"
import { TerminalHangStage, TerminalUserInterventionAction, telemetryService } from "@/services/telemetry"
import { ShowMessageType } from "@/shared/proto/index.host"
import { isBluCliInstalled, isCliSubagentContext } from "@/utils/cli-detector"
import { isInTestMode } from "../../services/test/TestMode"
import { refreshWorkflowToggles } from "../context/instructions/user-instructions/workflows"
import {
	ensureLocalBluDirExists
} from "../context/instructions/user-instructions/rule-helpers"
...
export class Task {
	...
	private bluIgnoreController: BluIgnoreController
	...
	constructor(params: TaskParams) {
		...
		this.bluIgnoreController = new BluIgnoreController(cwd)
		...
		const effectiveApiConfiguration: ApiConfiguration = {
			...
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				const bluMessages = this.messageStateHandler.getBluMessages()
				const lastApiReqStartedIndex = findLastIndex(bluMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: BluApiReqInfo = JSON.parse(bluMessages[lastApiReqStartedIndex].text || "{}")
						...
						await this.messageStateHandler.updateBluMessage(lastApiReqStartedIndex, {
	...
	async ask(
		...
		if (this.taskState.abort && type !== "resume_task" && type !== "resume_completed_task") {
			throw new Error("Blu instance aborted")
		}
		let askTs: number
		if (partial !== undefined) {
			const bluMessages = this.messageStateHandler.getBluMessages()
			const lastMessage = bluMessages.at(-1)
			const lastMessageIndex = bluMessages.length - 1
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.messageStateHandler.updateBluMessage(lastMessageIndex, {
						text,
						partial,
					})
					// todo be more efficient about saving and posting only new data or one whole message at a time so ignore partial for saves, and only post parts of partial message instead of whole array in new listener
					// await this.saveBluMessagesAndUpdateHistory()
					// await this.postStateToWebview()
					const protoMessage = convertBluMessageToProto(lastMessage)
					...
					await this.messageStateHandler.addToBluMessages({
						ts: askTs,
						type: "ask",
						ask: type,
						text,
			...
			// const lastMessage = this.bluMessages.at(-1)
			this.taskState.askResponse = undefined
			this.taskState.askResponseText = undefined
			this.taskState.askResponseImages = undefined
			this.taskState.askResponseFiles = undefined
	...
	async handleWebviewAskResponse(askResponse: BluAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async say(
		...
		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getBluMessages().at(-1)
			const isUpdatingPreviousPartial =
			...
			if (partial) {
				...
				if (isUpdatingPreviousPartial) {
					...
					await this.messageStateHandler.saveBluMessagesAndUpdateHistory()
					// await this.postStateToWebview()
					const protoMessage = convertBluMessageToProto(lastMessage)
	...
	async sayAndCreateMissingParamError(toolName: BluDefaultTool, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Blu tried to use ${toolName}${
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: BluAsk | BluSay) {
		const bluMessages = this.messageStateHandler.getBluMessages()
		const lastMessage = bluMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setBluMessages(bluMessages.slice(0, -1))
			await this.messageStateHandler.saveBluMessagesAndUpdateHistory()
		}
	}
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.bluIgnoreController.initialize()
		} catch (error) {
			console.error("Failed to initialize BluIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and bluMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the bluMessages might not be empty, so we need to set it to [] when we create a new Blu client (otherwise webview would show stale messages from previous session)
		this.messageStateHandler.setBluMessages([])
		...
		if (hooksEnabled) {
			...
			if (taskStartResult.cancel === true) {
				...
				if (taskStartResult.wasCancelled) {
					...
					// Set flag to allow Controller.cancelTask() to proceed
					this.taskState.didFinishAbortingStream = true
					// Save BOTH bluMessages AND apiConversationHistory so Controller.cancelTask() can find the task
					await this.messageStateHandler.saveBluMessagesAndUpdateHistory()
	...
	public async resumeTaskFromHistory() {
		...
		const savedBluMessages = await getSavedBluMessages(this.taskId)
		...
		if (lastRelevantMessageIndex !== -1) {
			savedBluMessages.splice(lastRelevantMessageIndex + 1)
		}

		// since we don't use api_req_finished anymore, we need to check if the last api_req_started has a cost value, if it doesn't and no cancellation reason to present, then we remove it since it indicates an api request without any partial content streamed
		const lastApiReqStartedIndex = findLastIndex(savedBluMessages, (m) => m.type === "say" && m.say === "api_req_started")
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savedBluMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: BluApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				savedBluMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.messageStateHandler.overwriteBluMessages(savedBluMessages)
		this.messageStateHandler.setBluMessages(await getSavedBluMessages(this.taskId))

		// Now present the blu messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		const lastBluMessage = this.messageStateHandler
			.getBluMessages()
			.slice()
			.reverse()
		...
		let askType: BluAsk
		if (lastBluMessage?.ask === "completion_result") {
			...
			const taskResumeResult = await executeHook({
				...
				hookInput: {
					taskResume: {
						...
						previousState: {
							lastMessageTs: lastBluMessage?.ts?.toString() || "",
							messageCount: bluMessages.length.toString(),
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with blu messages
		...
		const agoText = (() => {
			const timestamp = lastBluMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastBluMessage?.ts && Date.now() - lastBluMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.taskState.abort) {
			const didEndLoop = await this.recursivelyMakeBluRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that blu will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Blu responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async abortTask() {
		try {
			...
			this.bluIgnoreController.dispose()
	...
	async executeCommandTool(command: string, timeoutSeconds: number | undefined): Promise<[boolean, ToolResponse]> {
		// For Blu CLI subagents, we want to parse and process the command to ensure flags are correct
		const isSubagent = isSubagentCommand(command)

		if (transformBluCommand(command) !== command && isSubagent) {
			command = transformBluCommand(command)
		}
		...
		const clearCommandState = async () => {
			...
			const bluMessages = this.messageStateHandler.getBluMessages()
			const lastCommandIndex = findLastIndex(bluMessages, (m) => m.ask === "command" || m.say === "command")
			if (lastCommandIndex !== -1) {
				await this.messageStateHandler.updateBluMessage(lastCommandIndex, {
	...
	public async cancelHookExecution(): Promise<boolean> {
		...
		try {
			...
			const bluMessages = this.messageStateHandler.getBluMessages()
			const hookMessageIndex = bluMessages.findIndex((m) => m.ts === messageTs)
			if (hookMessageIndex !== -1) {
				...
				await this.messageStateHandler.updateBluMessage(hookMessageIndex, {
	...
	private async migrateDisableBrowserToolSetting(): Promise<void> {
		const config = vscode.workspace.getConfiguration("blu")
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		// blu browser tool uses image recognition for navigation (requires model image support).
		...
		if (subagentsEnabled) {
			const bluCliInstalled = await isBluCliInstalled()
			isSubagentsEnabledAndCliInstalled = subagentsEnabled && bluCliInstalled
		}

		const { globalToggles, localToggles } = await refreshBluRulesToggles(this.controller, this.cwd)
		...
		const globalBluRulesFileInstructions = await getGlobalBluRules(globalBluRulesFilePath, globalToggles)

		const localBluRulesFileInstructions = await getLocalBluRules(this.cwd, localToggles)
		...
		const bluIgnoreContent = this.bluIgnoreController.bluIgnoreContent
		let bluIgnoreInstructions: string | undefined
		if (bluIgnoreContent) {
			bluIgnoreInstructions = formatResponse.bluIgnoreInstructions(bluIgnoreContent)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			globalBluRulesFileInstructions,
			localBluRulesFileInstructions,
			localCursorRulesFileInstructions,
			localCursorRulesDirInstructions,
			localWindsurfRulesFileInstructions,
			bluIgnoreInstructions,
		...
		const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(
			this.messageStateHandler.getApiConversationHistory(),
			this.messageStateHandler.getBluMessages(),
			this.api,
			this.taskState.conversationHistoryDeletedRange,
			previousApiReqIndex,
			await ensureTaskDirectoryExists(this.taskId),
		...
		try {
			...
			const bluError = ErrorService.get().toBluError(error, model.id, providerId)

			// Capture provider failure telemetry using bluError
			ErrorService.get().logMessage(bluError.message)
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						bluError.message = "Context window exceeded. Click retry to truncate the conversation and try again."
						this.taskState.didAutomaticallyRetryFailedApiRequest = false
					}
				}

				const streamingFailedMessage = bluError.serialize()
				...
				if (lastApiReqStartedIndex !== -1) {
					...
					await this.messageStateHandler.updateBluMessage(lastApiReqStartedIndex, {
						...
						} satisfies BluApiReqInfo),
					})
					// this.ask will trigger postStateToWebview, so this change should be picked up.
				}

				// Check if this is a Blu provider insufficient credits error - don't auto-retry these
				const isBluProviderInsufficientCredits = (() => {
					if (providerId !== "blu") {
						return false
					}
					try {
						const parsedError = BluError.transform(error, model.id, providerId)
						return parsedError.isErrorType(BluErrorType.Balance)
					} catch {
						return false
					}
				})()

				let response: BluAskResponse
				// Skip auto-retry for Blu provider insufficient credits errors
				if (!isBluProviderInsufficientCredits && this.taskState.autoRetryAttempts < 3) {
					...
					if (!isBluProviderInsufficientCredits) {
						await this.say(
							"error_retry",
							JSON.stringify({
								attempt: 3,
				...
				if (manualRetryApiReqIndex !== -1) {
					const bluMessages = this.messageStateHandler.getBluMessages()
					const currentApiReqInfo: BluApiReqInfo = JSON.parse(bluMessages[manualRetryApiReqIndex].text || "{}")
					currentApiReqInfo.streamingFailedMessage = undefined
					await this.messageStateHandler.updateBluMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
	...
	async recursivelyMakeBluRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Blu is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images, files } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Blu uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4 Sonnet for its advanced agentic coding capabilities.",
			)
		...
		const previousApiReqIndex = findLastIndex(this.messageStateHandler.getBluMessages(), (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.messageStateHandler.getBluMessages().filter((m) => m.say === "api_req_started").length === 0
		...
		if (
			...
			try {
				...
				this.taskState.checkpointManagerErrorMessage = errorMessage // will be displayed right away since we saveBluMessages next which posts state to webview
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
			...
			if (lastCheckpointMessageIndex !== -1) {
				this.checkpointManager
					...
					.then(async (commitHash) => {
						if (commitHash) {
							await this.messageStateHandler.updateBluMessage(lastCheckpointMessageIndex, {
								lastCheckpointHash: commitHash,
							})
							// saveBluMessagesAndUpdateHistory will be called later after API response,
		...
		if (useAutoCondense && isNextGenModelFamily(this.api.getModel().id)) {
			...
			if (this.taskState.currentlySummarizing) {
				...
				// estimation, which require a full new message to be completed along with gathering the latest usage block. A proxy for whether
				...
				// approaches such as storing this.taskState.currentlySummarizing on disk in the bluMessages. This was intentionally not done
				// for now to prevent additional disk from needing to be used.
				// The worse case scenario is effectively blu summarizing a summary, which is bad UX, but doesn't break other logic.
			...
			if (shouldCompact) {
				...
				;[parsedUserContent, environmentDetails, blurulesError] = await this.loadContext(
					userContent,
					includeFileDetails,
				)
			}

			// error handling if the user uses the /newrule command & their .blurules is a file, for file read operations didnt work properly
			if (blurulesError === true) {
				await this.say(
					"error",
					"Issue with processing the /newrule command. Double check that, if '.blurules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
				)
			}
			...
			const [parsedUserContent, environmentDetails, blurulesError] = await this.loadContext(
				userContent,
				includeFileDetails,
				useCompactPrompt,
			)
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getBluMessages(), (m) => m.say === "api_req_started")
		await this.messageStateHandler.updateBluMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies BluApiReqInfo),
		})
		...
		try {
			...
			const abortStream = async (cancelReason: BluApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			try {
				for await (const chunk of stream) {
					...
					switch (chunk.type) {
						...
						case "tool_calls": {
							...
							if (chunk.tool_call.function?.id && chunk.tool_call.function?.name) {
								...
								if (chunk.tool_call.function.name.includes(BLU_MCP_TOOL_IDENTIFIER)) {
									this.taskState.toolUseIdMap.set(BluDefaultTool.MCP_USE, chunk.tool_call.function.id)
								}
							}
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of blu)
				...
				// abandoned happens when extension is no longer waiting for the blu instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
					const bluError = ErrorService.get().toBluError(error, this.api.getModel().id)
					const errorMessage = bluError.serialize()
			...
			// OpenRouter/Blu may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantMessage.length > 0 || this.useNativeToolCalls) {
				...
				if (assistantTextOnly.trim().length > 0) {
					assistantContent.push({
						...
						// reasoning_details only exists for blu/openrouter providers
						// @ts-ignore-next-line
				...
				const recDidEndLoop = await this.recursivelyMakeBluRequests(this.taskState.userMessageContent)
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that Blu cannot process. Retrying the request may help resolve this issue."
	...
	async loadContext(
		...
		// Track if we need to check blurulesFile
		let needsBlurulesFileCheck = false
				...
				userContent.map(async (block) => {
					if (block.type === "text") {
						...
						if (
							...
							const { processedText, needsBlurulesFileCheck: needsCheck } = await parseSlashCommands(
								parsedText,
		...
		// After processing content, check blurulesData if needed
		let blurulesError = false
		if (needsBlurulesFileCheck) {
			blurulesError = await ensureLocalBluDirExists(this.cwd, GlobalFileNames.bluRules)
		}
		...
		// Return all results
		return [processedUserContent, environmentDetails, blurulesError]
	}
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		// It could be useful for blu to know if the user went from one or no file to another between messages, so we always include this context
		...
		// Filter paths through bluIgnoreController
		const allowedVisibleFiles = this.bluIgnoreController
		...
		// Filter paths through bluIgnoreController
		const allowedOpenTabs = this.bluIgnoreController
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.bluIgnoreController)
				details += result
			}
		...
		const getTotalTokensFromApiReqMessage = (msg: BluMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const bluMessages = this.messageStateHandler.getBluMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(bluMessages.slice(1)))
