...
export class AttemptCompletionHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = BluDefaultTool.ATTEMPT
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const addNewChangesFlagToLastCompletionResultMessage = async () => {
			...
			const bluMessages = config.messageState.getBluMessages()

			const lastCompletionResultMessageIndex = findLastIndex(bluMessages, (m: any) => m.say === "completion_result")
			const lastCompletionResultMessage =
				lastCompletionResultMessageIndex !== -1 ? bluMessages[lastCompletionResultMessageIndex] : undefined
			if (
				...
				await config.messageState.updateBluMessage(lastCompletionResultMessageIndex, {
		...
		if (partialCompletionIndex !== -1) {
			const updatedMessages = [
				...bluMessages.slice(0, partialCompletionIndex),
				...bluMessages.slice(partialCompletionIndex + 1),
			]
			config.messageState.setBluMessages(updatedMessages)
			await config.messageState.saveBluMessagesAndUpdateHistory()
		}

		let commandResult: any
		const lastMessage = config.messageState.getBluMessages().at(-1)
		...
		if (config.messageState.getBluMessages().at(-1)?.ask === "command_output") {
