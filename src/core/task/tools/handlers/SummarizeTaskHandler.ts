...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = BluDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .bluignore first and skip ignored files
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(BluDefaultTool.FILE_READ, relPath)) {
						try {
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getBluMessages(),
				config.api,
