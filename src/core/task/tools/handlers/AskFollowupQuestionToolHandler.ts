...
import { BluAsk, BluAskQuestion } from "@shared/ExtensionMessage"
...
export class AskFollowupQuestionToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = BluDefaultTool.ASK
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		} satisfies BluAskQuestion

		await uiHelpers.ask("followup" as BluAsk, JSON.stringify(sharedMessage), block.partial).catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (optionsRaw && text && options.includes(text)) {
			...
			const lastFollowupMessage = findLast(bluMessages, (m: any) => m.ask === "followup")
