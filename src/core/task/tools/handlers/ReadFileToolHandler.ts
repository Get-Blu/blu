...
export class ReadFileToolHandler implements IFullyManagedTool {
	readonly name = BluDefaultTool.FILE_READ
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Check bluignore access
		const accessValidation = this.validator.checkBluIgnorePath(relPath!)
		if (!accessValidation.ok) {
			await config.callbacks.say("bluignore_error", relPath)
			return formatResponse.toolError(formatResponse.bluIgnoreError(relPath!))
		}
		...
		if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)) {
			...
			const notificationMessage = `Blu wants to read ${getWorkspaceBasename(absolutePath, "ReadFileToolHandler.notification")}`
