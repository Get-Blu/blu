...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = BluDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Blu wants to create a github issue...",
				message: `Blu is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const bluVersion = ExtensionRegistryInfo.version
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("blu-version", bluVersion)
				...
				await createAndOpenGitHubIssue("blu", "blu", "bug_report.yml", params)
			} catch (error) {
