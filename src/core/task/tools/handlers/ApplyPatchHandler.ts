...
class PathResolver {
	...
	validate(resolvedPath: string): { ok: boolean; error?: string } {
		return this.validator.checkBluIgnorePath(resolvedPath)
	}
...
export class ApplyPatchHandler implements IFullyManagedTool {
	readonly name = BluDefaultTool.APPLY_PATCH
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		try {
			...
			const autoApproveCheck = async (path: string, message: BluSayTool) => {
				const blockName = message.tool === "newFileCreated" ? BluDefaultTool.FILE_NEW : BluDefaultTool.FILE_EDIT
	...
	private async getOriginalFileContentForPreview(
		...
		const validation = this.validator.checkBluIgnorePath(resolution.resolvedPath)
		if (!validation.ok) {
			return undefined
		}
		...
		try {
			...
			for (const message of messages) {
				...
				for (const filePath of changedFiles) {
					config.services.fileContextTracker.markFileAsEditedByBlu(filePath)
					await config.services.fileContextTracker.trackFileContext(filePath, "blu_edited")
				}
	...
	private async generateChangeSummary(changes: Record<string, FileChange>): Promise<BluSayTool[]> {
	...
	private async handleApproval(
		...
		showNotificationForApproval(
			`Blu wants to apply a patch to ${fileCount} file(s)`,
