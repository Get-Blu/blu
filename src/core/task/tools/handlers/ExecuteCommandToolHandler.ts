...
import { BluAsk } from "@shared/ExtensionMessage"
...
export class ExecuteCommandToolHandler implements IFullyManagedTool {
	readonly name = BluDefaultTool.BASH
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		if (shouldAutoApprove) {
			...
			await uiHelpers
				.ask("command" as BluAsk, uiHelpers.removeClosingTag(block, "command", command), block.partial)
				.catch(() => {})
		}
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Check bluignore validation for command
		const ignoredFileAttemptedToAccess = config.services.bluIgnoreController.validateCommand(actualCommand)
		if (ignoredFileAttemptedToAccess) {
			await config.callbacks.say("bluignore_error", ignoredFileAttemptedToAccess)
			return formatResponse.toolError(formatResponse.bluIgnoreError(ignoredFileAttemptedToAccess))
		}
		...
		if ((!requiresApprovalPerLLM && autoApproveSafe) || (requiresApprovalPerLLM && autoApproveSafe && autoApproveAll)) {
			...
			showNotificationForApproval(
				`Blu wants to execute a command: ${actualCommand}`,
