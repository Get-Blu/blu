...
export class ToolResultUtils {
	...
	static pushToolResult(
		...
		if (typeof content === "string") {
			...
			// Get tool_use_id from map, or use "blu" as fallback for backward compatibility
			const toolUseId = toolUseIdMap?.get(block.name) || "blu"
	...
	private static createToolResultBlock(content: ToolResponse, id?: string) {
		// If id is "blu", we treat it as a plain text result for backward compatibility
	 ...
	 */
	static async askApprovalAndPushFeedback(type: BluAsk, completeMessage: string, config: TaskConfig) {
