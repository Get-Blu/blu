import type { BluAsk, BluSay } from "@shared/ExtensionMessage"
import type { BluDefaultTool } from "@shared/tools"
import type { BluAskResponse } from "@shared/WebviewMessage"
...
export interface StronglyTypedUIHelpers {
	// Core UI methods
	say: (type: BluSay, text?: string, images?: string[], files?: string[], partial?: boolean) => Promise<number | undefined>
	...
	removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: BluAsk | BluSay) => Promise<void>

	// Approval methods
	shouldAutoApproveTool: (toolName: BluDefaultTool) => boolean | [boolean, boolean]
	shouldAutoApproveToolWithPath: (toolName: BluDefaultTool, path?: string) => Promise<boolean>
	askApproval: (messageType: BluAsk, message: string) => Promise<boolean>

	// Telemetry and notifications
	captureTelemetry: (toolName: BluDefaultTool, autoApproved: boolean, approved: boolean) => void
...
export function createUIHelpers(config: TaskConfig): StronglyTypedUIHelpers {
	return {
		...
		shouldAutoApproveTool: (toolName: BluDefaultTool) => config.autoApprover.shouldAutoApproveTool(toolName),
		shouldAutoApproveToolWithPath: config.callbacks.shouldAutoApproveToolWithPath,
		askApproval: async (messageType: BluAsk, message: string): Promise<boolean> => {
			const { response } = await config.callbacks.ask(messageType, message, false)
			return response === "yesButtonClicked"
		},
		captureTelemetry: (toolName: BluDefaultTool, autoApproved: boolean, approved: boolean) => {
