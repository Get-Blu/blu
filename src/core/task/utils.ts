...
import { BluApiReqCancelReason, BluApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const bluMessages = params.messageStateHandler.getBluMessages()
	const currentApiReqInfo: BluApiReqInfo = JSON.parse(bluMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateBluMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
