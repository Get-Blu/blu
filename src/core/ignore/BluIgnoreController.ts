 ...
 * Designed to be instantiated once in Blu.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .bluignore files.
 */
export class BluIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	bluIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.bluIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .bluignore
		this.setupFileWatcher()
		await this.loadBluIgnore()
	}

	/**
	 * Set up the file watcher for .bluignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".bluignore")
		...
		this.fileWatcher.on("error", (error) => {
			console.error("Error watching .bluignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .bluignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadBluIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.bluIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".bluignore")
			} else {
				this.bluIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			console.error("Unexpected error loading .bluignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processBluIgnoreIncludes(content)
	 ...
	 */
	private async processBluIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			console.debug(`[BluIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .bluignore does not exist
		if (!this.bluIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .bluignore exists
		if (!this.bluIgnoreContent) {
			return undefined
		}
