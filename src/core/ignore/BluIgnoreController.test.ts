...
import { BluIgnoreController } from "./BluIgnoreController"
import "should"

describe("BluIgnoreController", () => {
	let tempDir: string
	let controller: BluIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .bluignore file
		await fs.writeFile(
			path.join(tempDir, ".bluignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new BluIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .bluignore file", async () => {
			const result = controller.validateAccess(".bluignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .bluignore", async () => {
			// Create a new .bluignore with comments
			await fs.writeFile(
				path.join(tempDir, ".bluignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .bluignore gracefully", async () => {
			// Create a new controller in a directory without .bluignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new BluIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .bluignore", async () => {
			await fs.writeFile(path.join(tempDir, ".bluignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .bluignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".bluignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .bluignore
			controller = new BluIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .bluignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .bluignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".bluignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".bluignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
