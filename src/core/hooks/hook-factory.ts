...
import { version as bluVersion } from "../../../package.json"
 ...
 * provide--the other common parameters like bluVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - bluVersion: Current Blu extension version
	 ...
	 * - userId: Blu user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		const workspaceRoots =
			StateManager.get()
				...
				?.map((root) => root.path) || []
		return {
			bluVersion: bluVersion,
			hookName: this.hookName,
			timestamp: Date.now().toString(),
			workspaceRoots,
			userId: getDistinctId(), // Always available: Blu User ID, machine ID, or generated UUID
			...params,
		}
	}
}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/Blu/Hooks/)
 * and workspace-specific hooks (from each workspace's .blurules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .blurules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Includes both global hooks (from ~/Documents/Blu/Hooks/) and workspace hooks
	 * (from .blurules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .blurules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .blurules directory path to search
