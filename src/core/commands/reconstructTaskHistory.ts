import { getSavedBluMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const bluMessages = await getSavedBluMessages(taskId)
		if (bluMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(bluMessages, metadata)
...
function extractTaskInformation(bluMessages: BluMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = bluMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = bluMessages.length > 0 ? bluMessages[0].ts : Date.now()
	...
	const apiReqMessages = bluMessages.filter((msg) => msg.type === "say" && msg.say === "api_req_started" && msg.text)
	...
	const messageSize = JSON.stringify(bluMessages).length
