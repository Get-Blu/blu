...
import { createBluAPI } from "./exports"
...
import { addToBlu } from "./core/controller/commands/addToBlu"
import { explainWithBlu } from "./core/controller/commands/explainWithBlu"
import { fixWithBlu } from "./core/controller/commands/fixWithBlu"
import { improveWithBlu } from "./core/controller/commands/improveWithBlu"
import { openGitHubRepo } from "./core/controller/commands/openGitHubRepo"
import { viewFileHistory } from "./core/controller/commands/viewFileHistory"
...
export async function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.OpenInEditor, async () => {
			console.log("[DEBUG] OpenInEditor command triggered")
			const sidebarInstance = WebviewProvider.getInstance() as any
			console.log("[DEBUG] Sidebar instance type:", sidebarInstance.constructor.name)
			console.log("[DEBUG] Has openInEditorTab method:", typeof sidebarInstance.openInEditorTab)
			// Call method to open in editor tab
			if (typeof sidebarInstance.openInEditorTab === "function") {
				console.log("[DEBUG] Calling openInEditorTab()...")
				await sidebarInstance.openInEditorTab()
				console.log("[DEBUG] openInEditorTab() completed")
			} else {
				// Fallback to sidebar view if method doesn't exist
				console.log("[DEBUG] openInEditorTab method not found, falling back to sidebar view")
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(WebviewProvider.getInstance().controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("Blu dev task commands registered")
			})
			.catch((error) => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Blu (Always available)
					const addAction = new vscode.CodeAction("Add to Blu", vscode.CodeActionKind.QuickFix)
					addAction.command = {
					...
					// Explain with Blu (Always available)
					const explainAction = new vscode.CodeAction("Explain with Blu", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with Blu",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Blu (Always available)
					const improveAction = new vscode.CodeAction("Improve with Blu", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with Blu",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Blu (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Blu", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithBlu,
							title: "Fix with Blu",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.AddToChat, async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			...
			await addToBlu(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.FixWithBlu, async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
			await fixWithBlu(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ExplainCode, async (range: vscode.Range) => {
			...
			await explainWithBlu(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ImproveCode, async (range: vscode.Range) => {
			...
			await improveWithBlu(context.controller, context.commandContext)
		}),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#BluWalkthrough`)
	...
	// Register the openGitHubRepo command handler
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.OpenGitHubRepo, async () => {
			await openGitHubRepo()
		}),
	)

	// Register the viewFileHistory command handler
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ViewFileHistory, async () => {
			await viewFileHistory()
		}),
	)
	...
	Logger.log("Blu extension activated")

	const testModeWatchers = await initializeTestMode(webview)
	// Initialize test mode and add disposables to context
	context.subscriptions.push(...testModeWatchers)

	vscode.commands.executeCommand("setContext", "blu.isDevMode", IS_DEV && IS_DEV === "true")
	...
	return createBluAPI(webview.controller)
}
...
function setupHostProvider(context: ExtensionContext) {
	...
	const outputChannel = vscode.window.createOutputChannel("Blu")
...
export async function deactivate() {
	Logger.log("Blu extension deactivating, cleaning up resources...")
	...
	Logger.log("Blu extension deactivated")
}
