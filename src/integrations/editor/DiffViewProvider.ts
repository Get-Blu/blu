...
import { DiagnosticSeverity, FileDiagnostics } from "@/shared/proto/index.blu"
...
export abstract class DiffViewProvider {
	...
	public async open(relPath: string, options?: { displayPath?: string }): Promise<void> {
		...
		// get diagnostics before editing the file, we'll compare to diagnostics after editing to see if blu needs to fix anything
	 ...
	 * Since these are new problems resulting from Blu's edit, we know they're
	 * directly related to the work he's doing. This eliminates the risk of Blu
	 * going off-task or getting distracted by unrelated issues, which was a problem
	 * with the previous auto-debug approach. Some users' machines may be slow to
	 * update diagnostics, so this approach provides a good balance between automation
	 * and avoiding potential issues where Blu might get stuck in loops due to
	 * outdated problem information. If no new problems show up by the time the user
	 * accepts the changes, they can always debug later using the '@problems' mention.
	 * This way, Blu only becomes aware of new problems resulting from his edits
	 * and can address them accordingly. If problems don't change immediately after
	 * applying a fix, Blu won't be notified, which is generally fine since the
