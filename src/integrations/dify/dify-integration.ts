 ...
 * with Blu's existing systems like file handling, conversation management,
...
export class DifyIntegration {
	 ...
	 * @param user User identifier (defaults to "blu-user")
...
/**
 * Utility function to convert Blu file objects to Dify upload format
 * @param files Array of file paths or file objects from Blu
 * @returns Promise with array of file data ready for upload
 */
export async function prepareBluFilesForDify(files: string[]): Promise<Array<{ name: string; content: Buffer }>> {
	...
	for (const filePath of files) {
		try {
			...
			const name = workspaceResolver.getBasename(filePath, "DifyIntegration.prepareBluFilesForDify")
