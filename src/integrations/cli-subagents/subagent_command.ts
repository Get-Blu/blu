/**
 * Pattern to match simplified Blu CLI syntax: blu "prompt" or blu 'prompt'
 * with optional additional flags after the closing quote
 */
const CLINE_COMMAND_PATTERN = /^blu\s+(['"])(.+?)\1(\s+.*)?$/

/**
 * Detects if a command is a Blu CLI subagent command.
 *
 * Matches the simplified syntax: blu "prompt" or blu 'prompt'
 * This allows the system to apply subagent-specific settings like autonomous execution.
 *
 * @param command - The command string to check
 * @returns True if the command is a Blu CLI subagent command, false otherwise
 */
...
/**
 * Transforms simplified Blu CLI command syntax with subagent settings.
 *
 * Converts: blu "prompt" or blu 'prompt'
 * To: blu "prompt" -s yolo_mode_toggled=true -s max_consecutive_mistakes=6 -F plain -y --oneshot
 *
 * Preserves additional flags like --workdir:
 * blu "prompt" --workdir ./path → blu "prompt" -s ... -F plain -y --oneshot --workdir ./path
 *
 ...
 */
export function transformBluCommand(command: string): string {
...
/**
 * Injects subagent-specific command structure and settings into Blu CLI commands.
 *
 * @param command - The Blu CLI command (simplified or full syntax)
 * @returns The command with injected flags and settings
 */
function injectSubagentSettings(command: string): string {
	// No pre-prompt flags needed - use standard "blu 'prompt'" syntax
	...
	if (match) {
		...
		return `blu ${prePromptPart}${quote}${prompt}${quote} ${postPromptFlags.join(" ")}${additionalFlags}`
	}
