...
import { BluApiReqInfo, BluMessage, BluSay } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { HistoryItem } from "@shared/HistoryItem"
import { BluCheckpointRestore } from "@shared/WebviewMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const bluMessages = this.services.messageStateHandler.getBluMessages()
			bluMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getBluMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.saveBluMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								console.error(
				...
				const lastFivebluMessages = this.services.messageStateHandler.getBluMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFivebluMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: BluCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const bluMessages = this.services.messageStateHandler.getBluMessages()
			const messageIndex = bluMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(bluMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = bluMessages[messageIndex]
			const lastMessageWithHash = bluMessages[lastHashIndex]
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			...
			const bluMessages = this.services.messageStateHandler.getBluMessages()
			const messageIndex = bluMessages.findIndex((m) => m.ts === messageTs)
			const message = bluMessages[messageIndex]
			if (!message) {
				console.error(`[TaskCheckpointManager] Message not found for timestamp ${messageTs} in task ${this.task.taskId}`)
				relinquishButton()
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getBluMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const bluMessages = this.services.messageStateHandler.getBluMessages()
			const messageIndex = findLastIndex(bluMessages, (m) => m.say === "completion_result")
			const message = bluMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const bluMessages = this.services.messageStateHandler.getBluMessages()
				const deletedMessages = bluMessages.slice(messageIndex + 1)
				...
				const newBluMessages = bluMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwriteBluMessages(newBluMessages) // calls saveBluMessages which saves historyItem
		...
		try {
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					...
					await this.setcheckpointManagerErrorMessage(
						"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Blu in a project that uses git, or disabling checkpoints.",
					)
				}
			}, 7_000)
			...
			const tracker = await pTimeout(
				CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath),
				{
					milliseconds: 15_000,
					message:
						"Checkpoints taking too long to initialize. Consider re-opening Blu in a project that uses git, or disabling checkpoints.",
				},
			)
			...
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage(
					"Checkpoints initialization timed out. Consider re-opening Blu in a project that uses git, or disabling checkpoints.",
				)
			} else {
				await this.setcheckpointManagerErrorMessage(errorMessage)
			}
