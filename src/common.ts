...
/**
 * Performs intialization for Blu that is common to all platforms.
 *
 * @param context
...
export async function initialize(context: vscode.ExtensionContext): Promise<WebviewProvider> {
	try {
		...
		HostProvider.window.showMessage({
			...
			message: "Failed to initialize Blu's application state. Please restart the extension.",
		})
	}
	...
	// Migrate custom instructions to global Blu rules (one-time cleanup)
...
async function showVersionUpdateAnnouncement(context: vscode.ExtensionContext) {
	...
	const previousVersion = context.globalState.get<string>("bluVersion")
	// Perform post-update actions if necessary
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Blu version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				// Focus Blu when there's a new announcement to show (major/minor updates or fresh installs)
				const message = previousVersion
					? `Blu has been updated to v${currentVersion}`
					: `Welcome to Blu v${currentVersion}`
				await HostProvider.workspace.openbluSidebarPanel({})
			...
			await context.globalState.update("bluVersion", currentVersion)
		}
	} catch (error) {
...
/**
 * Performs cleanup when Blu is deactivated that is common to all platforms.
 */
