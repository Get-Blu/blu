import { BluMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the BluSayHook interface but represents parsed data.
 */
 ...
 */
function isToolOrCommandMessage(msg: BluMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: BluMessage): HookMetadata | null {
 ...
 */
function filterPartialToolMessages(messages: BluMessage[]): BluMessage[] {
...
function combineHookWithOutputs(
	hookMessage: BluMessage,
	startIndex: number,
	messages: BluMessage[],
): { combined: BluMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: BluMessage[]): BluMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, BluMessage>()
 ...
 */
function findNextToolTimestamp(hookIndex: number, messages: BluMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: BluMessage[], originalMessages: BluMessage[]): Map<number, BluMessage[]> {
	const map = new Map<number, BluMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: BluMessage[], preToolUseMap: Map<number, BluMessage[]>): BluMessage[] {
	const result: BluMessage[] = []
 ...
 *
 * @param messages Array of BluMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: BluMessage[]): BluMessage[] {
