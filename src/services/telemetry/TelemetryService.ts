...
import { BluAccountUserInfo } from "@/services/auth/AuthService"
...
export type TelemetryMetadata = {
	/** The extension or blu-core version. */
...
/**
 * TelemetryService handles telemetry event tracking for the Blu extension
...
export class TelemetryService {
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual Blu rules are toggled on/off
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (hostSetting.isEnabled === Setting.DISABLED) {
			// Only show warning if user has opted in to Blu telemetry but host telemetry is disabled
			if (didUserOptIn) {
				void HostProvider.window
					.showMessage({
						type: ShowMessageType.WARNING,
						message:
							"Anonymous Blu error and usage reporting is enabled, but IDE telemetry is disabled. To enable error and usage reporting for this extension, enable telemetry in IDE settings.",
						options: {
							items: ["Open Settings"],
						},
					})
	 ...
	 */
	public identifyAccount(userInfo: BluAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when blu calls the task completion_result tool signifying that blu is done with the task
	...
	/**
	 * Records when individual Blu rules are toggled on/off
	 ...
	 */
	public captureBluRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
