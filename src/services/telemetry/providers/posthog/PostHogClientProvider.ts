...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the Blu extension.
	 * this is specifically to avoid capturing errors from anything other than Blu
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from Blu
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "blu"
			if (stacktrace?.value?.toLowerCase().includes("blu")) {
				return event
			}
			// Check stack frames for Blu extension path
