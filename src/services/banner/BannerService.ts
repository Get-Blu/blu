import type { Banner, BannerRules, BannersResponse } from "@shared/BluBanner"
import { isBluInternalTester } from "@shared/internal/account"
...
export class BannerService {
	...
	private async getIdeType(): Promise<string> {
		try {
			...
			// Use bluType field which contains values like "VSCode Extension", "Blu for JetBrains", etc.
			const bluType = hostVersion.bluType?.toLowerCase() || ""

			if (bluType.includes("vscode")) {
				return "vscode"
			}
			if (bluType.includes("jetbrains")) {
				return "jetbrains"
			}
	...
	/**
	 * Checks if the current user is a Blu employee
	 * @returns true if user has a @getblu.in email or is a trusted tester
	 */
	private isEmployee(): boolean {
		try {
			...
			return isBluInternalTester(authInfo.user.email)
		} catch (error) {
