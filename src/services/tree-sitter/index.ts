...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = bluIgnoreController ? bluIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, bluIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (bluIgnoreController && !bluIgnoreController.validateAccess(filePath)) {
		return null
	}
