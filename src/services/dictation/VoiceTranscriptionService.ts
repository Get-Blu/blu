...
const STATUS_ERROR_MAP = new Map<number, string>([
	[401, "Authentication failed. Please reauthenticate your Blu account"],
...
export class VoiceTranscriptionService {
	private readonly bluAccountService: BluAccountService

	constructor() {
		this.bluAccountService = BluAccountService.getInstance()
	}
	...
	private parseTranscriptionError(error: unknown): { error: string } {
		...
		if (axios.isAxiosError(error)) {
			...
			return {
				error: "Transcription failed. Please try again later or raise an issue on https://github.com/blu/blu/issues",
			}
		}
	...
	async transcribeAudio(audioBase64: string, language?: string): Promise<{ text?: string; error?: string }> {
		try {
			Logger.info("Transcribing audio with Blu transcription service...")

			// Check if using organization account for telemetry
			const userInfo = await this.bluAccountService.fetchMe()
			const activeOrg = userInfo?.organizations?.find((org) => org.active)
			const isOrgAccount = !!activeOrg

			const result = await this.bluAccountService.transcribeAudio(audioBase64, language)
