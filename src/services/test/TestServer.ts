import { getSavedApiConversationHistory, getSavedBluMessages } from "@core/storage/disk"
...
export async function createTestServer(controller: Controller): Promise<http.Server> {
	// Try to show the Blu sidebar
	Logger.log("[createTestServer] Opening Blu in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Blu instance found" }))
					return
				}
				...
				try {
					...
					// TODO: convert apiKey to bluAccountId
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "blu" as ApiProvider,
							bluAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.stateManager.setSecret("bluAccountId", apiKey)

						visibleWebview.controller.stateManager.setApiConfiguration(updatedConfig)

						// Update cache service to use blu provider
						...
						visibleWebview.controller.stateManager.setApiConfiguration({
							...currentConfig,
							planModeApiProvider: "blu",
							actModeApiProvider: "blu",
						})
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedBluMessages(taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Blu messages: ${error}`)
						}
