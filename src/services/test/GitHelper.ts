...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".blu_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Blu Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "blu@example.com"], { cwd: workspacePath })
