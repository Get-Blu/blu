...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// blu-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the blu-core process by:
...
function showHelp() {
	console.log(`
Blu Core - Standalone Server

Usage: node blu-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Blu data storage (default: ~/.blu)
