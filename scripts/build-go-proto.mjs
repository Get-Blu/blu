...
export async function goProtoc(outDir, protoFiles) {
	...
	const goProtocCommand = [
		...
		`--go_out="${outDir}"`,
		`--go_opt=module=github.com/blu/grpc-go`,
		`--go-grpc_out="${outDir}"`,
		`--go-grpc_opt=module=github.com/blu/grpc-go`,
		...protoFiles,
	].join(" ")
...
async function generateGoMod() {
	...
	const goModContent = `module github.com/blu/grpc-go
...
import (
	...
	"google.golang.org/grpc"
	"github.com/blu/grpc-go/client/services"
)

// BluClient provides a unified interface to all Blu services
...
// NewBluClient creates a new unified Blu client
func NewBluClient(address string) (*BluClient, error) {
	config := &ConnectionConfig{
		Address: address,
	}
...
// NewBluClientWithConfig creates a new Blu client with custom configuration
func NewBluClientWithConfig(config *ConnectionConfig) (*BluClient, error) {
	connManager := NewConnectionManager(config)
...
// Connect establishes connection to Blu Core and initializes service clients
func (c *BluClient) Connect(ctx context.Context) error {
	c.mutex.Lock()
...
// Disconnect closes the connection to Blu Core
func (c *BluClient) Disconnect() error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
// IsConnected returns true if the client is connected to Blu Core
func (c *BluClient) IsConnected() bool {
	c.mutex.RLock()
...
func (c *BluClient) Reconnect(ctx context.Context) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *BluClient) GetConnection() *grpc.ClientConn {
	return c.connManager.GetConnection()
}
`
	const clientPath = path.join(GO_CLIENT_DIR, "blu_client.go")
	...
	for (const [serviceName, serviceDef] of Object.entries(services)) {
		...
		const methods = serviceDef.methods
			.map((method) => {
				...
				// Determine if types are from blu package (common types) or proto package (service-specific types)
				const requestTypeName = method.requestType.split(".").pop()
				const responseTypeName = method.responseType.split(".").pop()

				// Common types like StringRequest, Empty, etc. are in the blu package
...
func (sc *${capitalizedServiceName}Client) ${capitalizedMethodName}(ctx context.Context, req ${requestType}) (${responseType}, error) {
	...
	return resp, nil
		...
		const protoImportPath =
			serviceDef.protoPackage === "host" ? '"github.com/blu/grpc-go/host"' : '"github.com/blu/grpc-go/blu"'

		// Check if we need to import blu package for common types
		const needsBluImport = serviceDef.methods.some((method) => {
			const requestTypeName = method.requestType.split(".").pop()
			const responseTypeName = method.responseType.split(".").pop()
		...
		// Always import blu package if we need common types, regardless of service package
		const bluImport = needsBluImport ? '	blu "github.com/blu/grpc-go/blu"\n' : ""
...
${bluImport}	proto ${protoImportPath}
	"google.golang.org/grpc"
)
