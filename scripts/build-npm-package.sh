...
# Script to build the Blu NPM package with telemetry keys injected
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Blu NPM Package Build Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if npm run compile-cli-all-platforms; then
  ...
  if ls cli/bin/blu-* 1> /dev/null 2>&1; then
    echo -e "${GREEN}✓ CLI binaries verified:${NC}"
    ls -lh cli/bin/blu-* | awk '{print "  " $9 " (" $5 ")"}'
  else
...
if grep -q "process.env.TELEMETRY_SERVICE_API_KEY" dist-standalone/blu-core.js; then
...
if grep -q "data.getblu.in" dist-standalone/blu-core.js; then
  # Extract a snippet of the PostHog config
  POSTHOG_CONFIG=$(grep -A 3 "data.getblu.in" dist-standalone/blu-core.js | head -5)
...
echo -e "2. Verify:        ${YELLOW}blu version${NC}"
echo -e "3. Publish:       ${YELLOW}cd dist-standalone && npm publish${NC}"
echo ""
echo -e "${YELLOW}Note: Check PostHog dashboard after running blu commands to verify telemetry${NC}"
