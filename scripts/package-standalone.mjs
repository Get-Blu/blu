...
async function main() {
	...
	console.log(`🚀 Building Blu ${buildType} Package\n`)
...
/**
 * Copy CLI binaries (blu and blu-host) for all platforms
...
async function copyCliBinaries() {
	...
	for (const { os, arch } of platforms) {
		...
		// Copy blu binary
		const bluSource = path.join(CLI_BINARIES_DIR, `blu-${platformSuffix}`)
		const bluDest = path.join(binDir, `blu-${platformSuffix}`)

		if (!fs.existsSync(bluSource)) {
			console.error(`Error: CLI binary not found at ${bluSource}`)
			console.error(`Please run: npm run compile-cli`)
			process.exit(1)
		}

		await cpr(bluSource, bluDest)
		fs.chmodSync(bluDest, 0o755)
		console.log(`✓ blu-${platformSuffix} copied`)

		// Copy blu-host binary
		const hostSource = path.join(CLI_BINARIES_DIR, `blu-host-${platformSuffix}`)
		const hostDest = path.join(binDir, `blu-host-${platformSuffix}`)
		...
		console.log(`✓ blu-host-${platformSuffix} copied`)
	}
 ...
 * The proto/descriptor_set.pb file is needed by blu-core for gRPC reflection
 */
 ...
 * Ripgrep is needed by blu-core for file searching
 */
async function copyRipgrepBinary() {
	...
	// Copy ripgrep binary to the root of dist-standalone (where blu-core.js is)
...
async function createNpmPackageFiles() {
	...
	const manPageSource = path.join("cli", "man", "blu.1")
	const manDir = path.join(BUILD_DIR, "man")
	const manPageDest = path.join(manDir, "blu.1")
...
function setupBinaries() {
	...
	console.log(\`Setting up Blu CLI for \${platformSuffix}...\`);

	const binDir = path.join(__dirname, 'bin');
	
	// Check if platform-specific binaries exist
	const bluSource = path.join(binDir, \`blu-\${platformSuffix}\`);
	const bluHostSource = path.join(binDir, \`blu-host-\${platformSuffix}\`);
	
	if (!fs.existsSync(bluSource)) {
		console.error(\`Error: Binary not found for platform \${platformSuffix}\`);
		console.error(\`Expected: \${bluSource}\`);
		console.error(\`Supported platforms: darwin-arm64, darwin-amd64, linux-amd64, linux-arm64\`);
		process.exit(1);
	}
	
	if (!fs.existsSync(bluHostSource)) {
		console.error(\`Error: Binary not found for platform \${platformSuffix}\`);
		console.error(\`Expected: \${bluHostSource}\`);
		process.exit(1);
	}

	// Create symlinks or copies to the generic names
	const bluTarget = path.join(binDir, 'blu');
	const bluHostTarget = path.join(binDir, 'blu-host');
	
	// Remove existing files if they exist
	[bluTarget, bluHostTarget].forEach(target => {
	...
	if (platform === 'win32') {
		// Windows: copy files
		fs.copyFileSync(bluSource, bluTarget);
		fs.copyFileSync(bluHostSource, bluHostTarget);
		console.log('✓ Copied platform-specific binaries');
	} else {
		// Unix: create symlinks
		fs.symlinkSync(path.basename(bluSource), bluTarget);
		fs.symlinkSync(path.basename(bluHostSource), bluHostTarget);
		console.log('✓ Created symlinks to platform-specific binaries');
		
		// Make binaries executable
		try {
			fs.chmodSync(bluSource, 0o755);
			fs.chmodSync(bluHostSource, 0o755);
			fs.chmodSync(bluTarget, 0o755);
			fs.chmodSync(bluHostTarget, 0o755);
		} catch (error) {
	...
	console.log('✓ Blu CLI installation complete');
	console.log('');
	console.log('Usage:');
	console.log('  blu        - Start Blu CLI');
	console.log('  blu-host   - Start Blu host service');
	console.log('');
	console.log('Documentation: https://docs.getblu.in');
}
...
try {
	...
	console.error('Please report this issue at: https://github.com/blu/blu/issues');
	process.exit(1);
}
`
 ...
 * When blu-core is installed, the installer should use the correct module for the current platform.
 */
async function packageAllBinaryDeps() {
	...
	for (const module of SUPPORTED_BINARY_MODULES) {
		...
		if (!fs.existsSync(src)) {
			console.warn(`Warning: Trying to install binaries for the module '${module}', but it is not being used by blu.`)
			continue
		}
...
async function zipDistribution() {
	...
	// Add the whole blu directory under "extension", except the for the ignored files.
