 ...
 *    - Changes name to "blu-nightly"
 *    - Changes displayName to "Blu (Nightly)"
...
const config = {
	...
	nightlyName: "blu-nightly",
	nightlyDisplayName: "Blu (Nightly)",
	...
	get vsixPath() {
		return path.join(this.distDir, "blu-nightly.vsix")
	},
}
...
class NightlyPublisher {
	 ...
	 */
	updatePackageJson() {
		// Replace any occurrences blu. or claude-dev with nightly name
		const rawContent = fs.readFileSync(config.packageJsonPath, "utf-8")
		const content = rawContent.replaceAll("claude-dev", config.nightlyName).replaceAll('"blu.', `"${config.nightlyName}.`)
