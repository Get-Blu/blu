 ...
 * ==================================
 *
 * This script generates Go code for the CLI version of Blu by extracting provider
...
//   4. Include universal fields (requestTimeoutMs, ulid, bluAccountId) for all providers
//
...
func getFieldsByProvider(providerID string, allFields []ConfigField, required bool) []ConfigField {
	...
	for _, field := range allFields {
		...
		if override, hasOverride := GetFieldOverride(providerID, field.Name); hasOverride {
			...
			universalFields := []string{"requesttimeoutms", "bluaccountid"}
