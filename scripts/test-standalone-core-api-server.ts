...
/**
 * Simple Blu gRPC Server
 *
 * This script provides a minimal way to run the Blu core gRPC service
 ...
 *   2. BluApiServerMock (mock implementation of the Blu API)
 ...
 *   CLINE_CORE_FILE - Override core file name (default: blu-core.js)
...
import { BluApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const bluCoreFile = process.env.CLINE_CORE_FILE || "blu-core.js"
const coreFile = path.join(distDir, bluCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple Blu gRPC Server...")
	...
	try {
		...
		console.log("Blu API Server started in-process")
	} catch (error) {
		console.error("Failed to start Blu API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const bluTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "blu-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("npx", ["tsx", path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: bluTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", path.join(distDir, "blu-core.js")]

	const spawnArgs = USE_C8 ? ["c8", "--report-dir", covDir, "node", ...baseArgs] : ["node", ...baseArgs]

	console.log(`Starting Blu Core Service... (useC8=${USE_C8})`)
	...
	const shutdown = async () => {
		...
		await BluApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(bluTestWorkspace, { recursive: true, force: true })
	...
	console.log(`Blu gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple Blu server:", err)
		process.exit(1)
	})
}
