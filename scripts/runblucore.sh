...
# This installs the blu-core app to the user's home directory,
...
LOG_FILE=~/.blu/blu-core-service.log
...
pkill -f blu-core.js || true

# Detect platform name using the same logic as BluDirs.kt in the plugin.
...
echo pwd: $(pwd)
set -x
NODE_PATH=$BINARY_MODULES_DIR:./node_modules DEV_WORKSPACE_FOLDER=/tmp/ node blu-core.js 2>&1 | tee $LOG_FILE
