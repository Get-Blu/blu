 ...
 * Returns array of:
 * { serviceName: string, serviceKey: string, protoPackage: "blu"|"host", moduleBase: string }
 */
async function parseServicesWithFiles(protoDir, protoFiles) {
	...
	for (const relPath of protoFiles) {
		...
		const pkg = relPath.startsWith("host/") ? "host" : "blu"
...
async function generateBluClientPy(outDir, services) {
...
class BluClient:
    """
    Unified Python client analogous to src/generated/grpc-go/client/BluClient.

    Usage:
        client = BluClient("localhost:17611")
        client.connect()
...
`
	...
	await fs.writeFile(path.join(clientDir, "blu_client.py"), content)
}
...
async function generatePythonClient(protoDir, pyOutDir, clientDir, protoFiles) {
	...
	// blu_client.py (unified that composes service wrappers)
	await generateBluClientPy(clientDir, services)
}
...
[project]
name = "blu-grpc-python"
version = "0.1.0"
description = "Generated Python gRPC stubs and client wrappers for Blu protos"
