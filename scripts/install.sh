...
# Configuration
INSTALL_DIR="${CLINE_INSTALL_DIR:-$HOME/.blu/cli}"
GITHUB_REPO="blu/blu"
...
check_existing_installation() {
    ...
    if [ -d "$INSTALL_DIR/bin" ] && [ -f "$INSTALL_DIR/bin/blu" ]; then
        # Extract version from blu binary
        local installed_version=$("$INSTALL_DIR/bin/blu" version 2>/dev/null | head -1 | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' || echo "")
        ...
        if [ -n "$installed_version" ] && [ "$installed_version" = "$cli_tag_version" ]; then
            echo ""
            print_ok "Blu ${MAGENTA}${BOLD}$installed_version${NC} already installed"
            echo ""
...
install_blu() {
    ...
    echo -e "${MAGENTA}${BOLD}"
    local package_file="$tmp_dir/blu.tar.gz"
    ...
    print_ok "Blu installed to ${MAGENTA}${BOLD}$INSTALL_DIR${NC}"
}
...
verify_installation() {
    ...
    local blu_bin="$INSTALL_DIR/bin/blu"
    
    if [ ! -f "$blu_bin" ]; then
        print_error "Binary not found at $blu_bin"
        exit 1
    fi
...
print_success() {
    ...
    echo ""
    print_message "$NC" "Run this to start using ${MAGENTA}${BOLD}blu${NC} immediately:"
    echo ""
