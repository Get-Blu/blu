...
const getBluVersion = () => {
	try {
		...
		const bluMatch = extensions.match(/claude-dev@(\d+\.\d+\.\d+)/)
		return bluMatch ? bluMatch[1] : "Not installed"
	} catch (_err) {
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/blu/blu/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&blu-version=${systemInfo.bluVersion}` +
		`&system-info=${
