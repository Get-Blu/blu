...
echo ""
echo -e "${MAGENTA}${BOLD}Installing Blu CLI from local build${NC}"
echo ""
...
# Always rebuild standalone to ensure latest blu-core.js
...
# Copy standalone package first (includes node_modules, blu-core.js, etc.)
...
mkdir -p "$INSTALL_DIR/bin"
cp "$PROJECT_ROOT/cli/bin/blu" "$INSTALL_DIR/bin/"
cp "$PROJECT_ROOT/cli/bin/blu-host" "$INSTALL_DIR/bin/"
...
chmod +x "$INSTALL_DIR/bin/blu"
chmod +x "$INSTALL_DIR/bin/blu-host"
...
echo ""
echo -e "Run this to start using ${MAGENTA}${BOLD}blu${NC} immediately:"
echo ""
