...
// Get the fully qualified name for a proto type, e.g. getFqn('StringRequest') returns 'blu.StringRequest'
...
export async function loadServicesFromProtoDescriptor() {
	...
	for (const [name, def] of Object.entries(proto.blu)) {
		if (def && "service" in def) {
			protobusServices[name] = def
		} else {
			addTypeNameToFqn(name, `proto.blu.${name}`)
		}
	}
