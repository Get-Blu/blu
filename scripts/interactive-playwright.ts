...
/**
 * Interactive Playwright launcher for the Blu VS Code extension.
 *
 * Overview:
 *  - Starts the mock Blu API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the Blu extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the Blu sidebar.
 ...
 *
 *   3. VS Code will launch with the Blu extension loaded and gRPC recording enabled.
 *
...
import { BluApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with Blu extension is now running!")
